...
<div id="wrapper_thongbao">
 <div id="layout" class="clearfix">
       <div id="contenttop" class="clearfix">
          <?php include 'include/header_calendar.php' ?>
       <!-- end contenttop --></div>
...
<?php
 ...
 $EndDateByNumberOfWeek = date ('Y-m-j' , $new_date );
    ...
    $calendar_user="SELECT * FROM `t_lichcongtac` WHERE (t_lichcongtac.FK_CONGTY IN ".$_SESSION['FK_CONGTY_EX'].") AND (t_lichcongtac.C_ACTIVE = '1')  AND ((`C_DATE_STAR` BETWEEN '".$beginday."' AND '".$EndDateByNumberOfWeek."')) ORDER BY C_DATE_STAR,C_HOUR_START"; 
    $rswrk1 = $conn->Execute($calendar_user);
    $arwrk1 = ($rswrk1) ? $rswrk1->GetRows() : array();
    foreach ($array_donvi as $wrk) 
        {
            if ($sFilterWrk <> "") $sFilterWrk .= " OR ";
            $sFilterWrk .= "(t_lichcongtac.C_PARTICIPANTS_ID LIKE '%" . ew_AdjustSql(trim($wrk)) . "x%')";
	}
    if ($sFilterWrk <> "") {$sFilterWrk = "(" . $sFilterWrk . ")"; }  
    if ($rswrk1) $rswrk1->Close();
    $calendar_parent="SELECT * FROM `t_lichcongtac` WHERE ".$sFilterWrk." AND (t_lichcongtac.FK_CONGTY NOT IN ".$_SESSION['FK_CONGTY_EX'].") AND (t_lichcongtac.C_PUBLISH = '2') AND ((`C_DATE_STAR` BETWEEN '".$beginday."' AND '".$EndDateByNumberOfWeek."')) ORDER BY C_DATE_STAR,C_HOUR_START"; 
