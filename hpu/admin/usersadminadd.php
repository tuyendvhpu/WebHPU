...
usersadmin_add.FormID = "fusersadminadd"; // form ID
...
usersadmin_add.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($usersadmin->C_TENDANGNHAP->FldCaption()) ?>");
...
<input type="text" name="x_C_TENDANGNHAP" id="x_C_TENDANGNHAP" title="<?php echo $usersadmin->C_TENDANGNHAP->FldTitle() ?>" size="30" maxlength="100" value="<?php echo $usersadmin->C_TENDANGNHAP->EditValue ?>"<?php echo $usersadmin->C_TENDANGNHAP->EditAttributes() ?>>
<?php } else { ?>
...
<?php if ($usersadmin->CurrentAction <> "F") { ?>
<input type="password" name="x_C_MATKHAU" id="x_C_MATKHAU" title="<?php echo $usersadmin->C_MATKHAU->FldTitle() ?>" size="30" maxlength="100"<?php echo $usersadmin->C_MATKHAU->EditAttributes() ?>>
<?php } else { ?>
...
<?php if ($usersadmin->CurrentAction <> "F") { ?>
<input type="text" name="x_C_HOTEN" id="x_C_HOTEN" title="<?php echo $usersadmin->C_HOTEN->FldTitle() ?>" size="50" maxlength="255" value="<?php echo $usersadmin->C_HOTEN->EditValue ?>"<?php echo $usersadmin->C_HOTEN->EditAttributes() ?>>
<?php } else { ?>
...
<?php if ($usersadmin->CurrentAction <> "F") { ?>
<input type="text" name="x_C_DIACHI" id="x_C_DIACHI" title="<?php echo $usersadmin->C_DIACHI->FldTitle() ?>" size="50" maxlength="255" value="<?php echo $usersadmin->C_DIACHI->EditValue ?>"<?php echo $usersadmin->C_DIACHI->EditAttributes() ?>>
<?php } else { ?>
...
class cusersadmin_add {
	...
	function Page_Main() {
		...
		if ($usersadmin->CurrentAction == "F") { // Confirm page
		  $usersadmin->RowType = EW_ROWTYPE_VIEW; // Render view type
		} else {
	...
	function RenderRow() {
		...
		if ($usersadmin->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$usersadmin->C_TENDANGNHAP->EditValue = ew_HtmlEncode($usersadmin->C_TENDANGNHAP->CurrentValue);
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!is_null($usersadmin->C_TENDANGNHAP->FormValue) && $usersadmin->C_TENDANGNHAP->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $usersadmin->C_TENDANGNHAP->FldCaption();
		}
	...
	function AddRow() {
		...
		if ($usersadmin->C_TENDANGNHAP->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(C_TENDANGNHAP = '" . ew_AdjustSql($usersadmin->C_TENDANGNHAP->CurrentValue) . "')";
		...
		// C_TENDANGNHAP
		$usersadmin->C_TENDANGNHAP->SetDbValueDef($rsnew, $usersadmin->C_TENDANGNHAP->CurrentValue, "", FALSE);
		...
		$usersadmin->FK_USERLEVELID->SetDbValueDef($rsnew, $usersadmin->FK_USERLEVELID->CurrentValue, NULL, FALSE);
                // FK_MACONGTY
		$usersadmin->FK_MACONGTY->SetDbValueDef($rsnew,0 , NULL, FALSE);
		...
		if ($AddRow) {
			$usersadmin->PK_NGUOIDUNG_ID->setDbValue($conn->Insert_ID());
			$rsnew['PK_NGUOIDUNG_ID'] = $usersadmin->PK_NGUOIDUNG_ID->DbValue;
