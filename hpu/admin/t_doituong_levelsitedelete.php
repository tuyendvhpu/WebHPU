...
// Create page object
$t_doituong_levelsite_delete = new ct_doituong_levelsite_delete();
$Page =& $t_doituong_levelsite_delete;

// Page init
$t_doituong_levelsite_delete->Page_Init();

// Page main
$t_doituong_levelsite_delete->Page_Main();
?>
...
// Create page object
var t_doituong_levelsite_delete = new ew_Page("t_doituong_levelsite_delete");

// page properties
t_doituong_levelsite_delete.PageID = "delete"; // page ID
t_doituong_levelsite_delete.FormID = "ft_doituong_levelsitedelete"; // form ID
var EW_PAGE_ID = t_doituong_levelsite_delete.PageID; // for backward compatibility

// extend page with Form_CustomValidate function
t_doituong_levelsite_delete.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_doituong_levelsite_delete.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_doituong_levelsite_delete.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_doituong_levelsite_delete.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
if ($rs = $t_doituong_levelsite_delete->LoadRecordset())
	$t_doituong_levelsite_deletelTotalRecs = $rs->RecordCount(); // Get record count
if ($t_doituong_levelsite_deletelTotalRecs <= 0) { // No record found, exit
	if ($rs)
		$rs->Close();
	$t_doituong_levelsite_delete->Page_Terminate("t_doituong_levelsitelist.php"); // Return to list
}
?>
<p><span class="phpmaker"><?php echo $Language->Phrase("Delete") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $t_doituong_levelsite->TableCaption() ?><br><br>
<a href="<?php echo $t_doituong_levelsite->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_doituong_levelsite_delete->ShowMessage();
?>
<form action="<?php echo ew_CurrentPage() ?>" method="post">
<p>
<input type="hidden" name="t" id="t" value="t_doituong_levelsite">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t_doituong_levelsite_delete->arRecKeys as $key) { ?>
...
<?php echo $t_doituong_levelsite->TableCustomInnerHtml ?>
	...
	<tr class="ewTableHeader">
		<td valign="top"><?php echo $t_doituong_levelsite->FK_CONGTY->FldCaption() ?></td>
		<td valign="top"><?php echo $t_doituong_levelsite->C_TYPE->FldCaption() ?></td>
		<td valign="top"><?php echo $t_doituong_levelsite->C_NAME->FldCaption() ?></td>
		<td valign="top"><?php echo $t_doituong_levelsite->C_ORDER->FldCaption() ?></td>
		<td valign="top"><?php echo $t_doituong_levelsite->C_STATUS->FldCaption() ?></td>
	</tr>
	</thead>
	<tbody>
<?php
$t_doituong_levelsite_delete->lRecCnt = 0;
$i = 0;
while (!$rs->EOF) {
	$t_doituong_levelsite_delete->lRecCnt++;

	// Set row properties
	$t_doituong_levelsite->CssClass = "";
	$t_doituong_levelsite->CssStyle = "";
	$t_doituong_levelsite->RowAttrs = array();
	$t_doituong_levelsite->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t_doituong_levelsite_delete->LoadRowValues($rs);

	// Render row
	$t_doituong_levelsite_delete->RenderRow();
?>
	<tr<?php echo $t_doituong_levelsite->RowAttributes() ?>>
		<td<?php echo $t_doituong_levelsite->FK_CONGTY->CellAttributes() ?>>
<div<?php echo $t_doituong_levelsite->FK_CONGTY->ViewAttributes() ?>><?php echo $t_doituong_levelsite->FK_CONGTY->ListViewValue() ?></div></td>
		<td<?php echo $t_doituong_levelsite->C_TYPE->CellAttributes() ?>>
<div<?php echo $t_doituong_levelsite->C_TYPE->ViewAttributes() ?>><?php echo $t_doituong_levelsite->C_TYPE->ListViewValue() ?></div></td>
		<td<?php echo $t_doituong_levelsite->C_NAME->CellAttributes() ?>>
<div<?php echo $t_doituong_levelsite->C_NAME->ViewAttributes() ?>><?php echo $t_doituong_levelsite->C_NAME->ListViewValue() ?></div></td>
		<td<?php echo $t_doituong_levelsite->C_ORDER->CellAttributes() ?>>
<div<?php echo $t_doituong_levelsite->C_ORDER->ViewAttributes() ?>><?php echo $t_doituong_levelsite->C_ORDER->ListViewValue() ?></div></td>
		<td<?php echo $t_doituong_levelsite->C_STATUS->CellAttributes() ?>>
<div<?php echo $t_doituong_levelsite->C_STATUS->ViewAttributes() ?>><?php echo $t_doituong_levelsite->C_STATUS->ListViewValue() ?></div></td>
	</tr>
<?php
	$rs->MoveNext();
}
...
<?php
$t_doituong_levelsite_delete->Page_Terminate();
?>
<?php
...
// Page class
//
class ct_doituong_levelsite_delete {
	...
	// Page object name
	var $PageObjName = 't_doituong_levelsite_delete';
	...
	//
	function ct_doituong_levelsite_delete() {
	...
	function Page_Main() {
		...
		if (@$_GET["PK_DOITUONG"] <> "") {
			...
			if (!is_numeric($t_doituong_levelsite->PK_DOITUONG->QueryStringValue))
				$this->Page_Terminate("t_doituong_levelsitelist.php"); // Prevent SQL injection, exit
			$sKey .= $t_doituong_levelsite->PK_DOITUONG->QueryStringValue;
		} else {
			$bSingleDelete = FALSE;
		}
		if ($bSingleDelete) {
		...
		if ($nKeySelected <= 0)
			$this->Page_Terminate("t_doituong_levelsitelist.php"); // No key specified, return to list
		...
		foreach ($this->arRecKeys as $sKey) {
			...
			if (!is_numeric($sKeyFld))
				$this->Page_Terminate("t_doituong_levelsitelist.php"); // Prevent SQL injection, return to list
			$sFilter .= "`PK_DOITUONG`=" . ew_AdjustSql($sKeyFld) . " AND ";
			if (substr($sFilter, -5) == " AND ") $sFilter = substr($sFilter, 0, strlen($sFilter)-5) . ") OR ";
		}
		if (substr($sFilter, -4) == " OR ") $sFilter = substr($sFilter, 0, strlen($sFilter)-4);

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t_doituong_levelsite class, t_doituong_levelsiteinfo.php

		$t_doituong_levelsite->CurrentFilter = $sFilter;

		// Get action
		if (@$_POST["a_delete"] <> "") {
			$t_doituong_levelsite->CurrentAction = $_POST["a_delete"];
		} else {
			$t_doituong_levelsite->CurrentAction = "D"; // Delete record directly
		}
		switch ($t_doituong_levelsite->CurrentAction) {
			case "D": // Delete
				$t_doituong_levelsite->SendEmail = TRUE; // Send email on delete success
				if ($this->DeleteRows()) { // delete rows
					$this->setMessage($Language->Phrase("DeleteSuccess")); // Set up success message
					$this->Page_Terminate($t_doituong_levelsite->getReturnUrl()); // Return to caller
				}
		}
	}
	...
	function DeleteRows() {
		...
		$DeleteRows = TRUE;
		$sWrkFilter = $t_doituong_levelsite->CurrentFilter;

		// Set up filter (SQL WHERE clause) and get return SQL
		// SQL constructor in t_doituong_levelsite class, t_doituong_levelsiteinfo.php

		$t_doituong_levelsite->CurrentFilter = $sWrkFilter;
		...
		if ($DeleteRows) {
			foreach ($rsold as $row) {
				$DeleteRows = $t_doituong_levelsite->Row_Deleting($row);
				if (!$DeleteRows) break;
			}
		}
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['PK_DOITUONG'];
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$DeleteRows = $conn->Execute($t_doituong_levelsite->DeleteSQL($row)); // Delete
				$conn->raiseErrorFn = '';
				if ($DeleteRows === FALSE)
					break;
				if ($sKey <> "") $sKey .= ", ";
		...
		if ($DeleteRows) {
			foreach ($rsold as $row) {
				$t_doituong_levelsite->Row_Deleted($row);
			}	
		}
		return $DeleteRows;
	}
