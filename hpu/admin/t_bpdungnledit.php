...
// Create page object
$t_bpdungnl_edit = new ct_bpdungnl_edit();
$Page =& $t_bpdungnl_edit;

// Page init
$t_bpdungnl_edit->Page_Init();

// Page main
$t_bpdungnl_edit->Page_Main();
?>
...
// Create page object
var t_bpdungnl_edit = new ew_Page("t_bpdungnl_edit");

// page properties
t_bpdungnl_edit.PageID = "edit"; // page ID
t_bpdungnl_edit.FormID = "ft_bpdungnledit"; // form ID
var EW_PAGE_ID = t_bpdungnl_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
t_bpdungnl_edit.ValidateForm = function(fobj) {
...
t_bpdungnl_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_bpdungnl_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_bpdungnl_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_bpdungnl_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_bpdungnl_edit->ShowMessage();
?>
<form name="ft_bpdungnledit" id="ft_bpdungnledit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return t_bpdungnl_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="t_bpdungnl">
...
<div<?php echo $t_bpdungnl->FK_MACONGTY->ViewAttributes() ?>><?php echo $t_bpdungnl->FK_MACONGTY->EditValue ?></div><input type="hidden" name="x_FK_MACONGTY" id="x_FK_MACONGTY" value="<?php echo ew_HtmlEncode($t_bpdungnl->FK_MACONGTY->CurrentValue) ?>">
<?php } else { ?>
...
<input type="hidden" name="x_PK_BPDUNGNL" id="x_PK_BPDUNGNL" value="<?php echo ew_HtmlEncode($t_bpdungnl->PK_BPDUNGNL->CurrentValue) ?>">
<p>
<?php if ($t_bpdungnl->CurrentAction <> "F") { // Confirm page ?>
...
<script language="JavaScript" type="text/javascript">
<!--
SendRequest(<?php echo $t_bpdungnl->FK_LOAINHIENLIEU->CurrentValue; ?>,'t_loainhienlieu','C_DONVI','PK_LOAINHIENLIEU')
...
<?php
$t_bpdungnl_edit->Page_Terminate();
?>
<?php
...
class ct_bpdungnl_edit {
	...
	// Page object name
	var $PageObjName = 't_bpdungnl_edit';
	...
	function Page_Init() {
		...
		if (!$Security->CanEdit() || !$Security->CanEditex()) {
			$Security->SaveLastUrl();
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$t_bpdungnl->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$t_bpdungnl->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$t_bpdungnl->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$t_bpdungnl->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($t_bpdungnl->PK_BPDUNGNL->CurrentValue == "")
			$this->Page_Terminate("t_bpdungnllist.php"); // Invalid key, return to list
		switch ($t_bpdungnl->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					$this->setMessage($Language->Phrase("noedit")); // No record found
					$this->Page_Terminate("t_bpdungnllist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$t_bpdungnl->SendEmail = TRUE; // Send email on update success
				if ($this->EditRow()) { // Update record based on key
					$this->setMessage($Language->Phrase("UpdateSuccess")); // Update success
					$sReturnUrl = $t_bpdungnl->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "t_bpdungnlview.php")
						$sReturnUrl = $t_bpdungnl->ViewUrl(); // View paging, return to View page directly
		...
		if ($t_bpdungnl->CurrentAction == "F") { // Confirm page
			$t_bpdungnl->RowType = EW_ROWTYPE_VIEW; // Render as View
		} else {
			$t_bpdungnl->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		}
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		if ($t_bpdungnl->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($t_bpdungnl->RowType == EW_ROWTYPE_EDIT) { // Edit row
			...
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$t_bpdungnl->FK_MACONGTY->EditValue = $rswrk->fields('C_TENCONGTY');
					$rswrk->Close();
				} else {
					$t_bpdungnl->FK_MACONGTY->EditValue = $t_bpdungnl->FK_MACONGTY->CurrentValue;
				}
			} else {
				$t_bpdungnl->FK_MACONGTY->EditValue = NULL;
			}
			$t_bpdungnl->FK_MACONGTY->CssStyle = "";
			$t_bpdungnl->FK_MACONGTY->CssClass = "";
			$t_bpdungnl->FK_MACONGTY->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $t_bpdungnl->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($t_bpdungnl->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$t_bpdungnl->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
