...
// Create page object
$t_lichcongtac_mainsite_add = new ct_lichcongtac_mainsite_add();
$Page =& $t_lichcongtac_mainsite_add;

// Page init
$t_lichcongtac_mainsite_add->Page_Init();

// Page main
$t_lichcongtac_mainsite_add->Page_Main();
?>
...
// Create page object
var t_lichcongtac_mainsite_add = new ew_Page("t_lichcongtac_mainsite_add");

// page properties
t_lichcongtac_mainsite_add.PageID = "add"; // page ID
t_lichcongtac_mainsite_add.FormID = "ft_lichcongtac_mainsiteadd"; // form ID
var EW_PAGE_ID = t_lichcongtac_mainsite_add.PageID; // for backward compatibility

// extend page with ValidateForm function
t_lichcongtac_mainsite_add.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_lichcongtac_mainsite->FK_SB_ID->FldCaption()) ?>");
...
t_lichcongtac_mainsite_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_lichcongtac_mainsite_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_lichcongtac_mainsite_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_lichcongtac_mainsite_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<a href="<?php echo $t_lichcongtac_mainsite->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_lichcongtac_mainsite_add->ShowMessage();
?>
<form name="ft_lichcongtac_mainsiteadd" id="ft_lichcongtac_mainsiteadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return t_lichcongtac_mainsite_add.ValidateForm(this);">
<p>
...
IF (IsAdmin())
{
        if (is_array($t_lichcongtac_mainsite->FK_CONGTY->EditValue)) {
            ...
            for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
                    $selwrk = (strval($t_lichcongtac_mainsite->FK_CONGTY->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
if (is_array($t_lichcongtac_mainsite->FK_SB_ID->EditValue)) {
	...
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($t_lichcongtac_mainsite->FK_SB_ID->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
<?php if ($t_lichcongtac_mainsite->C_PARTICIPANTS_ID->Visible) { // C_PARTICIPANTS_ID ?>
 ...
 <select tabindex="-1" class="x_C_PARTICIPANTS_ID" multiple="multiple" style="width:300px" style="width:300px;" id="x_C_PARTICIPANTS_ID[]" name="x_C_PARTICIPANTS_ID[]" title="<?php echo $t_lichcongtac_mainsite->C_PARTICIPANTS_ID->FldTitle() ?>" multiple="multiple"<?php echo $t_lichcongtac_mainsite->C_PARTICIPANTS_ID->EditAttributes() ?>>
<?php
...
<?php
$t_lichcongtac_mainsite_add->Page_Terminate();
?>
<?php
...
// Page class
//
class ct_lichcongtac_mainsite_add {
	...
	// Page object name
	var $PageObjName = 't_lichcongtac_mainsite_add';
	...
	//
	function ct_lichcongtac_mainsite_add() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $t_lichcongtac_mainsite->CurrentAction = $_POST["a_add"]; // Get form action
		  ...
		  if ($bCopy) {
		    $t_lichcongtac_mainsite->CurrentAction = "C"; // Copy record
		  } else {
		    $t_lichcongtac_mainsite->CurrentAction = "I"; // Display blank record
		...
		switch ($t_lichcongtac_mainsite->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$t_lichcongtac_mainsite->SendEmail = TRUE; // Send email on add success
		    if ($this->AddRow()) { // Add successful
		      $this->setMessage($Language->Phrase("AddSuccess")); // Set up success message
					...
					if (ew_GetPageName($sReturnUrl) == "t_lichcongtac_mainsiteview.php")
						$sReturnUrl = $t_lichcongtac_mainsite->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$t_lichcongtac_mainsite->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $t_lichcongtac_mainsite;
		$t_lichcongtac_mainsite->C_OPTION->CurrentValue = 0;
		$t_lichcongtac_mainsite->C_FILE_ATTACH->CurrentValue = NULL; // Clear file related field
		$t_lichcongtac_mainsite->C_PUBLISH->CurrentValue = 0;
                $t_lichcongtac_mainsite->FK_SB_ID->CurrentValue = 3;
	}
	...
	function LoadFormValues() {
		...
		$t_lichcongtac_mainsite->C_ACTIVE->setFormValue($objForm->GetValue("x_C_ACTIVE"));
		$t_lichcongtac_mainsite->C_DATETIME_ACTIVE->setFormValue($objForm->GetValue("x_C_DATETIME_ACTIVE"));
		$t_lichcongtac_mainsite->C_DATETIME_ACTIVE->CurrentValue = ew_UnFormatDateTime($t_lichcongtac_mainsite->C_DATETIME_ACTIVE->CurrentValue, 7);
		...
		$t_lichcongtac_mainsite->C_USER_ADD->setFormValue($objForm->GetValue("x_C_USER_ADD"));
		$t_lichcongtac_mainsite->C_ADD_TIME->setFormValue($objForm->GetValue("x_C_ADD_TIME"));
		$t_lichcongtac_mainsite->C_ADD_TIME->CurrentValue = ew_UnFormatDateTime($t_lichcongtac_mainsite->C_ADD_TIME->CurrentValue, 7);
	...
	function RestoreFormValues() {
		...
		$t_lichcongtac_mainsite->C_ACTIVE->CurrentValue = $t_lichcongtac_mainsite->C_ACTIVE->FormValue;
		$t_lichcongtac_mainsite->C_DATETIME_ACTIVE->CurrentValue = $t_lichcongtac_mainsite->C_DATETIME_ACTIVE->FormValue;
		$t_lichcongtac_mainsite->C_DATETIME_ACTIVE->CurrentValue = ew_UnFormatDateTime($t_lichcongtac_mainsite->C_DATETIME_ACTIVE->CurrentValue, 7);
		...
		$t_lichcongtac_mainsite->C_USER_ADD->CurrentValue = $t_lichcongtac_mainsite->C_USER_ADD->FormValue;
		$t_lichcongtac_mainsite->C_ADD_TIME->CurrentValue = $t_lichcongtac_mainsite->C_ADD_TIME->FormValue;
		$t_lichcongtac_mainsite->C_ADD_TIME->CurrentValue = ew_UnFormatDateTime($t_lichcongtac_mainsite->C_ADD_TIME->CurrentValue, 7);
	}
	...
	function RenderRow() {
		...
		} elseif ($t_lichcongtac_mainsite->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function ValidateForm() {
		...
		if (!is_null($t_lichcongtac_mainsite->FK_SB_ID->FormValue) && $t_lichcongtac_mainsite->FK_SB_ID->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac_mainsite->FK_SB_ID->FldCaption();
		}
...
//		}
	...
	function AddRow() {
		...
		// FK_CONGTY
		$t_lichcongtac_mainsite->FK_CONGTY->SetDbValueDef($rsnew, $t_lichcongtac_mainsite->FK_CONGTY->CurrentValue, NULL, FALSE);

		// FK_SB_ID
		$t_lichcongtac_mainsite->FK_SB_ID->SetDbValueDef($rsnew, $t_lichcongtac_mainsite->FK_SB_ID->CurrentValue, 0, FALSE);
		...
		$t_lichcongtac_mainsite->C_STATUS_STAR->SetDbValueDef($rsnew, $t_lichcongtac_mainsite->C_STATUS_STAR->CurrentValue, NULL, FALSE);
                ...
                if (!isset($t_lichcongtac_mainsite->C_STATUS_STAR->CurrentValue))
                {
                // C_HOUR_START
		...
		$t_lichcongtac_mainsite->C_STATUS_END->SetDbValueDef($rsnew, $t_lichcongtac_mainsite->C_STATUS_END->CurrentValue, NULL, FALSE);
                ...
                if (!isset($t_lichcongtac_mainsite->C_STATUS_END->CurrentValue))
                {
                // C_HOUR_END
		...
		// C_ACTIVE
		$t_lichcongtac_mainsite->C_ACTIVE->SetDbValueDef($rsnew, $t_lichcongtac_mainsite->C_ACTIVE->CurrentValue, NULL, FALSE);

		// C_DATETIME_ACTIVE
		$t_lichcongtac_mainsite->C_DATETIME_ACTIVE->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
		$rsnew['C_DATETIME_ACTIVE'] =& $t_lichcongtac_mainsite->C_DATETIME_ACTIVE->DbValue;
		...
		// C_USER_ADD
		$t_lichcongtac_mainsite->C_USER_ADD->SetDbValueDef($rsnew, CurrentUserID(), NULL);
		$rsnew['C_USER_ADD'] =& $t_lichcongtac_mainsite->C_USER_ADD->DbValue;

		// C_ADD_TIME
		$t_lichcongtac_mainsite->C_ADD_TIME->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
		$rsnew['C_ADD_TIME'] =& $t_lichcongtac_mainsite->C_ADD_TIME->DbValue;

		// Call Row Inserting event
		$bInsertRow = $t_lichcongtac_mainsite->Row_Inserting($rsnew);
		if ($bInsertRow) {
			...
			$conn->raiseErrorFn = 'ew_ErrorFn';
                        echo $t_lichcongtac_mainsite->InsertSQL($rsnew);

			$AddRow = $conn->Execute($t_lichcongtac_mainsite->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$t_lichcongtac_mainsite->C_CADENLAR_ID->setDbValue($conn->Insert_ID());
			$rsnew['C_CADENLAR_ID'] = $t_lichcongtac_mainsite->C_CADENLAR_ID->DbValue;

			// Call Row Inserted event
			$t_lichcongtac_mainsite->Row_Inserted($rsnew);
		}
