...
// Create page object
$t_doituong_svtuonglai_add = new ct_doituong_svtuonglai_add();
$Page =& $t_doituong_svtuonglai_add;

// Page init
$t_doituong_svtuonglai_add->Page_Init();

// Page main
$t_doituong_svtuonglai_add->Page_Main();
?>
...
// Create page object
var t_doituong_svtuonglai_add = new ew_Page("t_doituong_svtuonglai_add");

// page properties
t_doituong_svtuonglai_add.PageID = "add"; // page ID
t_doituong_svtuonglai_add.FormID = "ft_doituong_svtuonglaiadd"; // form ID
var EW_PAGE_ID = t_doituong_svtuonglai_add.PageID; // for backward compatibility

// extend page with ValidateForm function
t_doituong_svtuonglai_add.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_doituong_svtuonglai->C_URL->FldCaption()) ?>");
...
t_doituong_svtuonglai_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_doituong_svtuonglai_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_doituong_svtuonglai_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_doituong_svtuonglai_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_doituong_svtuonglai_add->ShowMessage();
?>
<form name="ft_doituong_svtuonglaiadd" id="ft_doituong_svtuonglaiadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return t_doituong_svtuonglai_add.ValidateForm(this);">
<p>
...
<?php
$t_doituong_svtuonglai_add->Page_Terminate();
?>
<?php
...
// Page class
//
class ct_doituong_svtuonglai_add {
	...
	// Page object name
	var $PageObjName = 't_doituong_svtuonglai_add';
	...
	//
	function ct_doituong_svtuonglai_add() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $t_doituong_svtuonglai->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$t_doituong_svtuonglai->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $t_doituong_svtuonglai->CurrentAction = "C"; // Copy record
		  } else {
		    $t_doituong_svtuonglai->CurrentAction = "I"; // Display blank record
		...
		switch ($t_doituong_svtuonglai->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$t_doituong_svtuonglai->SendEmail = TRUE; // Send email on add success
		    if ($this->AddRow()) { // Add successful
		      $this->setMessage($Language->Phrase("AddSuccess")); // Set up success message
					...
					if (ew_GetPageName($sReturnUrl) == "t_doituong_svtuonglaiview.php")
						$sReturnUrl = $t_doituong_svtuonglai->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$t_doituong_svtuonglai->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadFormValues() {
		...
		$t_doituong_svtuonglai->C_USER_ADD->setFormValue($objForm->GetValue("x_C_USER_ADD"));
		$t_doituong_svtuonglai->C_TIME_ADD->setFormValue($objForm->GetValue("x_C_TIME_ADD"));
		$t_doituong_svtuonglai->C_TIME_ADD->CurrentValue = ew_UnFormatDateTime($t_doituong_svtuonglai->C_TIME_ADD->CurrentValue, 7);
	...
	function RestoreFormValues() {
		...
		$t_doituong_svtuonglai->C_USER_ADD->CurrentValue = $t_doituong_svtuonglai->C_USER_ADD->FormValue;
		$t_doituong_svtuonglai->C_TIME_ADD->CurrentValue = $t_doituong_svtuonglai->C_TIME_ADD->FormValue;
		$t_doituong_svtuonglai->C_TIME_ADD->CurrentValue = ew_UnFormatDateTime($t_doituong_svtuonglai->C_TIME_ADD->CurrentValue, 7);
	}
	...
	function RenderRow() {
		...
		} elseif ($t_doituong_svtuonglai->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		// C_BELONGTO
		//$t_doituong_svtuonglai->C_BELONGTO->SetDbValueDef($rsnew, $t_doituong_svtuonglai->C_BELONGTO->CurrentValue, NULL, FALSE);
		...
		// C_USER_ADD
		$t_doituong_svtuonglai->C_USER_ADD->SetDbValueDef($rsnew, CurrentUserID(), NULL);
		$rsnew['C_USER_ADD'] =& $t_doituong_svtuonglai->C_USER_ADD->DbValue;

		// C_TIME_ADD
		$t_doituong_svtuonglai->C_TIME_ADD->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
		$rsnew['C_TIME_ADD'] =& $t_doituong_svtuonglai->C_TIME_ADD->DbValue;

		// Call Row Inserting event
		$bInsertRow = $t_doituong_svtuonglai->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($t_doituong_svtuonglai->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$t_doituong_svtuonglai->DTSVTUONGLAI_ID->setDbValue($conn->Insert_ID());
			$rsnew['DTSVTUONGLAI_ID'] = $t_doituong_svtuonglai->DTSVTUONGLAI_ID->DbValue;

			// Call Row Inserted event
			$t_doituong_svtuonglai->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
