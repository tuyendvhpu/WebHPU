...
// Create page object
$t_congty_edit = new ct_congty_edit();
$Page =& $t_congty_edit;

// Page init
$t_congty_edit->Page_Init();

// Page main
$t_congty_edit->Page_Main();
?>
...
// Create page object
var t_congty_edit = new ew_Page("t_congty_edit");

// page properties
t_congty_edit.PageID = "edit"; // page ID
t_congty_edit.FormID = "ft_congtyedit"; // form ID
var EW_PAGE_ID = t_congty_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
t_congty_edit.ValidateForm = function(fobj) {
...
t_congty_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_congty_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_congty_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_congty_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $t_congty->TableCaption() ?><br><br>
<a href="<?php echo $t_congty->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_congty_edit->ShowMessage();
?>
<form name="ft_congtyedit" id="ft_congtyedit" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return t_congty_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="t_congty">
...
<div id="new_x_C_LOGO">
<?php if (!empty($t_congty->C_LOGO->Upload->DbValue)) { ?>
<label><input type="radio" name="a_C_LOGO" id="a_C_LOGO" value="1" checked="checked"><?php echo $Language->Phrase("Keep") ?></label>&nbsp;
<label><input type="radio" name="a_C_LOGO" id="a_C_LOGO" value="2"><?php echo $Language->Phrase("Remove") ?></label>&nbsp;
<label><input type="radio" name="a_C_LOGO" id="a_C_LOGO" value="3"><?php echo $Language->Phrase("Replace") ?><br></label>
<?php $t_congty->C_LOGO->EditAttrs["onchange"] = "this.form.a_C_LOGO[2].checked=true;" . @$t_congty->C_LOGO->EditAttrs["onchange"]; ?>
<?php } else { ?>
<input type="hidden" name="a_C_LOGO" id="a_C_LOGO" value="3">
<?php } ?>
...
<?php } ?>
<?php } ?>
<?php if (!ew_Empty($t_congty->C_LOGO->Upload->DbValue) && $t_congty->C_LOGO->Upload->Action == "3") echo "[" . $Language->Phrase("Old") . "]"; ?>
</div>
<div id="new_x_C_LOGO">
...
?>
<?php if (!ew_Empty($t_congty->C_LOGO->Upload->DbValue) && $t_congty->C_LOGO->Upload->Action == "3") echo "[" . $Language->Phrase("New") . "]"; ?>
</div>
<?php } ?>
...
<input type="hidden" name="x_PK_MACONGTY" id="x_PK_MACONGTY" value="<?php echo ew_HtmlEncode($t_congty->PK_MACONGTY->CurrentValue) ?>">
<p>
<?php if ($t_congty->CurrentAction <> "F") { // Confirm page ?>
...
class ct_congty_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$t_congty->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->GetUploadFiles(); // Get upload files
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$t_congty->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$t_congty->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$t_congty->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($t_congty->PK_MACONGTY->CurrentValue == "")
			$this->Page_Terminate("t_congtylist.php"); // Invalid key, return to list
		switch ($t_congty->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$t_congty->SendEmail = TRUE; // Send email on update success
				if ($this->EditRow()) { // Update record based on key
					$this->setMessage($Language->Phrase("UpdateSuccess")); // Update success
					$sReturnUrl = $t_congty->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "t_congtyview.php")
						$sReturnUrl = $t_congty->ViewUrl(); // View paging, return to View page directly
		...
		if ($t_congty->CurrentAction == "F") { // Confirm page
			$t_congty->RowType = EW_ROWTYPE_VIEW; // Render as View
		} else {
			$t_congty->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		}
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($t_congty->RowType == EW_ROWTYPE_EDIT) { // Edit row
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$t_congty->C_LOGO->Upload->SaveToSession(); // Save file value to Session
						if ($t_congty->C_LOGO->Upload->Action == "2" || $t_congty->C_LOGO->Upload->Action == "3") { // Update/Remove
			...
			$bUpdateRow = $t_congty->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($t_congty->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$t_congty->Row_Updated($rsold, $rsnew);
		$rs->Close();
