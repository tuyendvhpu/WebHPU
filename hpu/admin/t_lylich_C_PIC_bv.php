...
// Create page object
$t_lylich_C_PIC_blobview = new ct_lylich_C_PIC_blobview();
$Page =& $t_lylich_C_PIC_blobview;

// Page init
$t_lylich_C_PIC_blobview->Page_Init();

// Page main
$t_lylich_C_PIC_blobview->Page_Main();
?>
<?php
$t_lylich_C_PIC_blobview->Page_Terminate();
?>
<?php
...
// Page class
//
class ct_lylich_C_PIC_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page object name
	var $PageObjName = 't_lylich_C_PIC_blobview';
	...
	//
	function ct_lylich_C_PIC_blobview() {
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel('t_lylich');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('t_lylich', 'x_C_PIC');
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('C_PIC'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('C_PIC'));
				}
				$objBinary->Value = $rs->fields('C_PIC');
