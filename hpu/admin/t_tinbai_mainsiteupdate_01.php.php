...
class ct_tinbai_mainsite_update {
	...
	function LoadMultiUpdateValues() {
		...
		if ($rs = $this->LoadRecordset()) {
			...
			while (!$rs->EOF) {
				if ($i == 1) {
                                        $C_USER_ADD =$rs->fields('C_USER_ADD');
                                        $C_ADD_TIME =$rs->fields('C_ADD_TIME'); 
			   ...
			   $conn->raiseErrorFn = 'ew_ErrorFn';
                        if ($t_tinbai_mainsite->C_ACTIVE_MAINSITE->CurrentValue == 1)
                        {
                                ...
                                if (strval($t_tinbai_mainsite->FK_ARRAY_CONGTY->CurrentValue) <> "") {
                                        $arwrk = explode(",", $t_tinbai_mainsite->FK_ARRAY_CONGTY->CurrentValue);
                                        foreach ($arwrk as $wrk) {
                                                $sql = "INSERT INTO t_tinbai_mainlevel (PK_TINBAI_ID,FK_CONGTY,FK_EDITOR_ID,C_USER_ADD,C_ADD_TIME) VALUES ( '".$rs->fields('PK_TINBAI_ID')."','". ew_AdjustSql(trim($wrk))."','-20','".$C_USER_ADD."','".$C_ADD_TIME."') ";	
