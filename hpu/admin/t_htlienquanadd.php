...
// Create page object
$t_htlienquan_add = new ct_htlienquan_add();
$Page =& $t_htlienquan_add;

// Page init
$t_htlienquan_add->Page_Init();

// Page main
$t_htlienquan_add->Page_Main();
?>
...
// Create page object
var t_htlienquan_add = new ew_Page("t_htlienquan_add");

// page properties
t_htlienquan_add.PageID = "add"; // page ID
t_htlienquan_add.FormID = "ft_htlienquanadd"; // form ID
var EW_PAGE_ID = t_htlienquan_add.PageID; // for backward compatibility

// extend page with ValidateForm function
t_htlienquan_add.ValidateForm = function(fobj) {
...
t_htlienquan_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_htlienquan_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_htlienquan_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_htlienquan_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_htlienquan_add->ShowMessage();
?>
<form name="ft_htlienquanadd" id="ft_htlienquanadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return t_htlienquan_add.ValidateForm(this);">
<p>
...
<input type="text" name="x_C_NAME" id="x_C_NAME" title="<?php echo $t_htlienquan->C_NAME->FldTitle() ?>" size="80" maxlength="255" value="<?php echo $t_htlienquan->C_NAME->EditValue ?>"<?php echo $t_htlienquan->C_NAME->EditAttributes() ?>>
...
<input type="text" name="x_C_URL" id="x_C_URL" title="<?php echo $t_htlienquan->C_URL->FldTitle() ?>" size="80" maxlength="255" value="<?php echo $t_htlienquan->C_URL->EditValue ?>"<?php echo $t_htlienquan->C_URL->EditAttributes() ?>>
...
<?php
$t_htlienquan_add->Page_Terminate();
?>
<?php
...
class ct_htlienquan_add {
	...
	// Page object name
	var $PageObjName = 't_htlienquan_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $t_htlienquan->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->GetUploadFiles(); // Get upload files
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$t_htlienquan->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $t_htlienquan->CurrentAction = "C"; // Copy record
		  } else {
		    $t_htlienquan->CurrentAction = "I"; // Display blank record
		...
		switch ($t_htlienquan->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$t_htlienquan->SendEmail = TRUE; // Send email on add success
		    if ($this->AddRow()) { // Add successful
		      $this->setMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $t_htlienquan->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "t_htlienquanview.php")
						$sReturnUrl = $t_htlienquan->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$t_htlienquan->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadFormValues() {
		...
		$t_htlienquan->C_USER_ADD->setFormValue($objForm->GetValue("x_C_USER_ADD"));
		$t_htlienquan->C_ADD_TIME->setFormValue($objForm->GetValue("x_C_ADD_TIME"));
		$t_htlienquan->C_ADD_TIME->CurrentValue = ew_UnFormatDateTime($t_htlienquan->C_ADD_TIME->CurrentValue, 7);
	...
	function RestoreFormValues() {
		...
		$t_htlienquan->C_USER_ADD->CurrentValue = $t_htlienquan->C_USER_ADD->FormValue;
		$t_htlienquan->C_ADD_TIME->CurrentValue = $t_htlienquan->C_ADD_TIME->FormValue;
		$t_htlienquan->C_ADD_TIME->CurrentValue = ew_UnFormatDateTime($t_htlienquan->C_ADD_TIME->CurrentValue, 7);
	}
	...
	function RenderRow() {
		...
		} elseif ($t_htlienquan->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		// C_USER_ADD
		$t_htlienquan->C_USER_ADD->SetDbValueDef($rsnew, CurrentUserID(), NULL);
		$rsnew['C_USER_ADD'] =& $t_htlienquan->C_USER_ADD->DbValue;

		// C_ADD_TIME
		$t_htlienquan->C_ADD_TIME->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
		$rsnew['C_ADD_TIME'] =& $t_htlienquan->C_ADD_TIME->DbValue;

		// Call Row Inserting event
		$bInsertRow = $t_htlienquan->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($t_htlienquan->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$t_htlienquan->SYS_ID->setDbValue($conn->Insert_ID());
			$rsnew['SYS_ID'] = $t_htlienquan->SYS_ID->DbValue;

			// Call Row Inserted event
			$t_htlienquan->Row_Inserted($rsnew);
		}
