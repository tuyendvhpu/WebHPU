...
// Create page object
$t_thongbao_levelsite_add = new ct_thongbao_levelsite_add();
$Page =& $t_thongbao_levelsite_add;

// Page init
$t_thongbao_levelsite_add->Page_Init();

// Page main
$t_thongbao_levelsite_add->Page_Main();
?>
...
// Create page object
var t_thongbao_levelsite_add = new ew_Page("t_thongbao_levelsite_add");

// page properties
t_thongbao_levelsite_add.PageID = "add"; // page ID
t_thongbao_levelsite_add.FormID = "ft_thongbao_levelsiteadd"; // form ID
var EW_PAGE_ID = t_thongbao_levelsite_add.PageID; // for backward compatibility

// extend page with ValidateForm function
t_thongbao_levelsite_add.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_thongbao_levelsite->FK_DOITUONG_ID->FldCaption()) ?>");
...
t_thongbao_levelsite_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_thongbao_levelsite_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_thongbao_levelsite_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_thongbao_levelsite_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_thongbao_levelsite_add->ShowMessage();
?>
<form name="ft_thongbao_levelsiteadd" id="ft_thongbao_levelsiteadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return t_thongbao_levelsite_add.ValidateForm(this);">
<p>
...
<?php $t_thongbao_levelsite->FK_CONGTY_ID->EditAttrs["onchange"] = "ew_UpdateOpt('x_FK_DOITUONG_ID','x_FK_CONGTY_ID',true); " . @$t_thongbao_levelsite->FK_CONGTY_ID->EditAttrs["onchange"]; ?>
...
<?php if ($t_thongbao_levelsite->FK_DOITUONG_ID->Visible) { // FK_DOITUONG_ID ?>
	<tr<?php echo $t_thongbao_levelsite->FK_DOITUONG_ID->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_thongbao_levelsite->FK_DOITUONG_ID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
		<td<?php echo $t_thongbao_levelsite->FK_DOITUONG_ID->CellAttributes() ?>><span id="el_FK_DOITUONG_ID">
<div id="tp_x_FK_DOITUONG_ID" class="<?php echo EW_ITEM_TEMPLATE_CLASSNAME ?>"><label><input type="radio" name="x_FK_DOITUONG_ID" id="x_FK_DOITUONG_ID" title="<?php echo $t_thongbao_levelsite->FK_DOITUONG_ID->FldTitle() ?>" value="{value}"<?php echo $t_thongbao_levelsite->FK_DOITUONG_ID->EditAttributes() ?>></label></div>
<div id="dsl_x_FK_DOITUONG_ID" repeatcolumn="5">
<?php
$arwrk = $t_thongbao_levelsite->FK_DOITUONG_ID->EditValue;
if (is_array($arwrk)) {
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($t_thongbao_levelsite->FK_DOITUONG_ID->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;

		// Note: No spacing within the LABEL tag
?>
<?php echo ew_RepeatColumnTable($rowswrk, $rowcntwrk, 5, 1) ?>
<label><input type="radio" name="x_FK_DOITUONG_ID" id="x_FK_DOITUONG_ID" title="<?php echo $t_thongbao_levelsite->FK_DOITUONG_ID->FldTitle() ?>" value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?><?php echo $t_thongbao_levelsite->FK_DOITUONG_ID->EditAttributes() ?>><?php echo $arwrk[$rowcntwrk][1] ?></label>
...
</span><?php echo $t_thongbao_levelsite->FK_DOITUONG_ID->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php
$t_thongbao_levelsite_add->Page_Terminate();
?>
<?php
...
// Page class
//
class ct_thongbao_levelsite_add {
	...
	// Page object name
	var $PageObjName = 't_thongbao_levelsite_add';
	...
	//
	function ct_thongbao_levelsite_add() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $t_thongbao_levelsite->CurrentAction = $_POST["a_add"]; // Get form action
		  ...
		  if ($bCopy) {
		    $t_thongbao_levelsite->CurrentAction = "C"; // Copy record
		  } else {
		    $t_thongbao_levelsite->CurrentAction = "I"; // Display blank record
		...
		switch ($t_thongbao_levelsite->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$t_thongbao_levelsite->SendEmail = TRUE; // Send email on add success
		    if ($this->AddRow()) { // Add successful
		      $this->setMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $t_thongbao_levelsite->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "t_thongbao_levelsiteview.php")
						$sReturnUrl = $t_thongbao_levelsite->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$t_thongbao_levelsite->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $t_thongbao_levelsite;
		//$t_thongbao_levelsite->FK_DOITUONG_ID->CurrentValue = 2;
	}
	...
	function LoadFormValues() {
		...
		$t_thongbao_levelsite->FK_DOITUONG_ID->setFormValue($objForm->GetValue("x_FK_DOITUONG_ID"));
	...
	function RestoreFormValues() {
		...
		$t_thongbao_levelsite->FK_DOITUONG_ID->CurrentValue = $t_thongbao_levelsite->FK_DOITUONG_ID->FormValue;
	...
	function RenderRow() {
		...
		if ($t_thongbao_levelsite->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($t_thongbao_levelsite->RowType == EW_ROWTYPE_ADD) { // Add row
			...
			// FK_DOITUONG_ID
			$t_thongbao_levelsite->FK_DOITUONG_ID->EditCustomAttributes = "";
				$sFilterWrk = "";
			...
			$arwrk = ($rswrk) ? $rswrk->GetRows() : array();
			if ($rswrk) $rswrk->Close();
			$t_thongbao_levelsite->FK_DOITUONG_ID->EditValue = $arwrk;
	...
	function ValidateForm() {
		...
		if ($t_thongbao_levelsite->FK_DOITUONG_ID->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_thongbao_levelsite->FK_DOITUONG_ID->FldCaption();
		}
	...
	function AddRow() {
		...
		// FK_DOITUONG_ID
		$t_thongbao_levelsite->FK_DOITUONG_ID->SetDbValueDef($rsnew, $t_thongbao_levelsite->FK_DOITUONG_ID->CurrentValue, NULL, FALSE);
		...
		// C_USER_ADD
		$t_thongbao_levelsite->C_USER_ADD->SetDbValueDef($rsnew, CurrentUserID(), NULL);
		$rsnew['C_USER_ADD'] =& $t_thongbao_levelsite->C_USER_ADD->DbValue;

		// C_EDIT_TIME
		$t_thongbao_levelsite->C_ADD_TIME->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
		$rsnew['C_ADD_TIME'] =& $t_thongbao_levelsite->C_ADD_TIME->DbValue;

		// Call Row Inserting event
		$bInsertRow = $t_thongbao_levelsite->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($t_thongbao_levelsite->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$t_thongbao_levelsite->PK_THONGBAO->setDbValue($conn->Insert_ID());
			$rsnew['PK_THONGBAO'] = $t_thongbao_levelsite->PK_THONGBAO->DbValue;

			// Call Row Inserted event
			$t_thongbao_levelsite->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
