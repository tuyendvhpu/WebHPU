...
// Create page object
$t_thongbao_mainsite_edit = new ct_thongbao_mainsite_edit();
$Page =& $t_thongbao_mainsite_edit;

// Page init
$t_thongbao_mainsite_edit->Page_Init();

// Page main
$t_thongbao_mainsite_edit->Page_Main();
?>
...
// Create page object
var t_thongbao_mainsite_edit = new ew_Page("t_thongbao_mainsite_edit");

// page properties
t_thongbao_mainsite_edit.PageID = "edit"; // page ID
t_thongbao_mainsite_edit.FormID = "ft_thongbao_mainsiteedit"; // form ID
var EW_PAGE_ID = t_thongbao_mainsite_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
t_thongbao_mainsite_edit.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_thongbao_mainsite->FK_CONGTY_ID->FldCaption()) ?>");
...
t_thongbao_mainsite_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_thongbao_mainsite_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_thongbao_mainsite_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_thongbao_mainsite_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_thongbao_mainsite_edit->ShowMessage();
?>
<form name="ft_thongbao_mainsiteedit" id="ft_thongbao_mainsiteedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return t_thongbao_mainsite_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="t_thongbao_mainsite">
...
<input type="hidden" name="x_PK_THONGBAO" id="x_PK_THONGBAO" value="<?php echo ew_HtmlEncode($t_thongbao_mainsite->PK_THONGBAO->CurrentValue) ?>">
<p>
...
<?php
$t_thongbao_mainsite_edit->Page_Terminate();
?>
<?php
...
// Page class
//
class ct_thongbao_mainsite_edit {
	...
	// Page object name
	var $PageObjName = 't_thongbao_mainsite_edit';
	...
	//
	function ct_thongbao_mainsite_edit() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$t_thongbao_mainsite->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$t_thongbao_mainsite->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$t_thongbao_mainsite->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$t_thongbao_mainsite->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($t_thongbao_mainsite->PK_THONGBAO->CurrentValue == "")
			$this->Page_Terminate("t_thongbao_mainsitelist.php"); // Invalid key, return to list
		switch ($t_thongbao_mainsite->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$t_thongbao_mainsite->SendEmail = TRUE; // Send email on update success
				if ($this->EditRow()) { // Update record based on key
					$this->setMessage($Language->Phrase("UpdateSuccess")); // Update success
					$sReturnUrl = $t_thongbao_mainsite->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "t_thongbao_mainsiteview.php")
						$sReturnUrl = $t_thongbao_mainsite->ViewUrl(); // View paging, return to View page directly
		...
		// Render the record
		$t_thongbao_mainsite->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function LoadFormValues() {
		...
		$t_thongbao_mainsite->C_USER_EDIT->setFormValue($objForm->GetValue("x_C_USER_EDIT"));
		$t_thongbao_mainsite->C_EDIT_TIME->setFormValue($objForm->GetValue("x_C_EDIT_TIME"));
		$t_thongbao_mainsite->C_EDIT_TIME->CurrentValue = ew_UnFormatDateTime($t_thongbao_mainsite->C_EDIT_TIME->CurrentValue, 7);
	}
	...
	function RestoreFormValues() {
		...
		$t_thongbao_mainsite->C_END_DATE->CurrentValue = ew_UnFormatDateTime($t_thongbao_mainsite->C_END_DATE->CurrentValue, 7);
		$t_thongbao_mainsite->C_USER_EDIT->CurrentValue = $t_thongbao_mainsite->C_USER_EDIT->FormValue;
		$t_thongbao_mainsite->C_EDIT_TIME->CurrentValue = $t_thongbao_mainsite->C_EDIT_TIME->FormValue;
		$t_thongbao_mainsite->C_EDIT_TIME->CurrentValue = ew_UnFormatDateTime($t_thongbao_mainsite->C_EDIT_TIME->CurrentValue, 7);
	}
	...
	function RenderRow() {
		...
		if ($t_thongbao_mainsite->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// PK_THONGBAO
			$t_thongbao_mainsite->PK_THONGBAO->EditCustomAttributes = "";
			$t_thongbao_mainsite->PK_THONGBAO->EditValue = $t_thongbao_mainsite->PK_THONGBAO->CurrentValue;
			$t_thongbao_mainsite->PK_THONGBAO->CssStyle = "";
			$t_thongbao_mainsite->PK_THONGBAO->CssClass = "";
			$t_thongbao_mainsite->PK_THONGBAO->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// C_USER_EDIT
			$t_thongbao_mainsite->C_USER_EDIT->SetDbValueDef($rsnew, CurrentUserID(), NULL);
			$rsnew['C_USER_EDIT'] =& $t_thongbao_mainsite->C_USER_EDIT->DbValue;

			// C_EDIT_TIME
			$t_thongbao_mainsite->C_EDIT_TIME->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
			$rsnew['C_EDIT_TIME'] =& $t_thongbao_mainsite->C_EDIT_TIME->DbValue;
