...
<table border="0" width="100%" id="table806" cellspacing="0" cellpadding="0">
							<tr>
								...
								<?php echo $Language->Phrase("ChangePwdPage") ?></font></b></td>
...
class cchangepwd {
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel('t_nguoidung');
	...
	//
	function Page_Main() {
		global $conn, $Language, $Security, $gsFormError, $t_nguoidung;
		if (!ew_IsHttpPost())
			return;
		$bPwdUpdated = FALSE;
		...
		if ($this->ValidateForm($sOPwd, $sNPwd, $sCPwd)) {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					if ((EW_MD5_PASSWORD && md5($sOPwd) == $rs->fields('C_MATKHAU')) ||
						(!EW_MD5_PASSWORD && $sOPwd == $rs->fields('C_MATKHAU'))) {
						$rsnew = array('C_MATKHAU' => $sNPwd); // Change Password
						$sEmail = $rs->fields('C_EMAIL');
						$rs->Close();
						$conn->raiseErrorFn = 'ew_ErrorFn';
						$bValidPwd = $conn->Execute($t_nguoidung->UpdateSQL($rsnew));
						$conn->raiseErrorFn = '';
						if ($bValidPwd)
							$bPwdUpdated = TRUE;
					}	else {
