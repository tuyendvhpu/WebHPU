...
// Create page object
$t_event_add = new ct_event_add();
$Page =& $t_event_add;

// Page init
$t_event_add->Page_Init();

// Page main
$t_event_add->Page_Main();
?>
...
// Create page object
var t_event_add = new ew_Page("t_event_add");

// page properties
t_event_add.PageID = "add"; // page ID
t_event_add.FormID = "ft_eventadd"; // form ID
var EW_PAGE_ID = t_event_add.PageID; // for backward compatibility

// extend page with ValidateForm function
t_event_add.ValidateForm = function(fobj) {
...
t_event_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_event_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_event_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_event_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_event_add->ShowMessage();
?>
<form name="ft_eventadd" id="ft_eventadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return t_event_add.ValidateForm(this);">
<p>
...
class ct_event_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $t_event->CurrentAction = $_POST["a_add"]; // Get form action
		  ...
		  if ($bCopy) {
		    $t_event->CurrentAction = "C"; // Copy record
		  } else {
		    $t_event->CurrentAction = "I"; // Display blank record
		...
		switch ($t_event->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$t_event->SendEmail = TRUE; // Send email on add success
		    if ($this->AddRow()) { // Add successful
		      $this->setMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $t_event->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "t_eventview.php")
						$sReturnUrl = $t_event->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$t_event->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $t_event;
		$t_event->C_POST->CurrentValue = 1;
		$t_event->C_SEND_MAIL->CurrentValue = 0;
	}
	...
	function RenderRow() {
		...
		} elseif ($t_event->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$t_event->C_CONTENT_MAIL->SetDbValueDef($rsnew, $t_event->C_CONTENT_MAIL->CurrentValue, NULL, FALSE);
                ...
                // send Mail 
                                        ...
                                        $noidung = $t_event->C_EVENT_NAME->CurrentValue." - Loại sự kiện:".$t_event->C_TYPE_EVENT->CurrentValue;
		...
		$bInsertRow = $t_event->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($t_event->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$t_event->C_EVENT_ID->setDbValue($conn->Insert_ID());
			$rsnew['C_EVENT_ID'] = $t_event->C_EVENT_ID->DbValue;

			// Call Row Inserted event
			$t_event->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
