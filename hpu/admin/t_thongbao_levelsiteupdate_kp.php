	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_thongbao_levelsite->C_NOTICE_HIT->FldCaption()) ?>");
		        ...
		        if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_thongbao_levelsite->C_FILEANH->FldCaption()) ?>");
                }
...
?>
<form name="ft_thongbao_levelsiteupdate" id="ft_thongbao_levelsiteupdate" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return t_thongbao_levelsite_update.ValidateForm(this);">
<p>
...
<?php if ($t_thongbao_levelsite->PK_CHUYENMUC_ID->Visible) { // PK_CHUYENMUC_ID ?>
	<tr<?php echo $t_thongbao_levelsite->PK_CHUYENMUC_ID->RowAttributes ?>>
		<td<?php echo $t_thongbao_levelsite->PK_CHUYENMUC_ID->CellAttributes() ?>><?php echo $t_thongbao_levelsite->PK_CHUYENMUC_ID->FldCaption() ?></td>
		<td<?php echo $t_thongbao_levelsite->PK_CHUYENMUC_ID->CellAttributes() ?>><span id="el_PK_CHUYENMUC_ID">
<select id="x_PK_CHUYENMUC_ID" name="x_PK_CHUYENMUC_ID" title="<?php echo $t_thongbao_levelsite->PK_CHUYENMUC_ID->FldTitle() ?>"<?php echo $t_thongbao_levelsite->PK_CHUYENMUC_ID->EditAttributes() ?>>
<?php
if (is_array($t_thongbao_levelsite->PK_CHUYENMUC_ID->EditValue)) {
	$arwrk = $t_thongbao_levelsite->PK_CHUYENMUC_ID->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($t_thongbao_levelsite->PK_CHUYENMUC_ID->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
</span><?php echo $t_thongbao_levelsite->PK_CHUYENMUC_ID->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php } ?>
        <tr>
            <td>Xác nhận gửi mail cho đơn vị thiết kế</td>
            <td>
...
<input type="text" name="x_C_KEYWORD" id="x_C_KEYWORD" title="<?php echo $t_thongbao_levelsite->C_KEYWORD->FldTitle() ?>" size="80" maxlength="255" value="<?php echo $t_thongbao_levelsite->C_KEYWORD->EditValue ?>"<?php echo $t_thongbao_levelsite->C_KEYWORD->EditAttributes() ?>>
...
<?php } ?> 
 <?php if ($t_thongbao_levelsite->C_FILEANH->Visible) { // C_FILEANH ?>
	<tr<?php echo $t_thongbao_levelsite->C_FILEANH->RowAttributes ?>>
		<td<?php echo $t_thongbao_levelsite->C_FILEANH->CellAttributes() ?>><?php echo $t_thongbao_levelsite->C_FILEANH->FldCaption() ?></td>
		<td<?php echo $t_thongbao_levelsite->C_FILEANH->CellAttributes() ?>><span id="el_C_FILEANH">
<input type="text" name="x_C_FILEANH" id="x_C_FILEANH" title="<?php echo $t_thongbao_levelsite->C_FILEANH->FldTitle() ?>" size="75" maxlength="255" value="<?php echo $t_thongbao_levelsite->C_FILEANH->EditValue ?>"<?php echo $t_thongbao_levelsite->C_FILEANH->EditAttributes() ?>>
</span><?php echo $t_thongbao_levelsite->C_FILEANH->CustomMsg ?>
   <i>  (Size ảnh hiển thị tốt nhất: 640px* 344px) -- <a target="_blank" href="http://img.hpu.edu.vn/index.php?/category/2145"> Kho ảnh thông báo sự kiện HPU  </a></i>               
                </td>
	</tr>
<?php } ?>
...
class ct_thongbao_levelsite_update {
	...
	function LoadMultiUpdateValues() {
		global $t_thongbao_levelsite,$C_FILEANH;
			...
			while (!$rs->EOF) {
				if ($i == 1) {
					...
					$t_thongbao_levelsite->C_EDIT_TIME->setDbValue($rs->fields('C_EDIT_TIME'));
                                        ...
                                        $t_thongbao_levelsite->PK_CHUYENMUC_ID->setDbValue($rs->fields('PK_CHUYENMUC_ID'));
                                        $t_thongbao_levelsite->C_FILEANH->setDbValue($rs->fields('C_FILEANH'));
					...
					if (!ew_CompareValue($t_thongbao_levelsite->C_KEYWORD->DbValue, $rs->fields('C_KEYWORD')))
						...
						$t_thongbao_levelsite->FK_DOITUONG_ID->CurrentValue = NULL;
                                        if (!ew_CompareValue($t_thongbao_levelsite->PK_CHUYENMUC_ID->DbValue, $rs->fields('PK_CHUYENMUC_ID')))
						$t_thongbao_levelsite->PK_CHUYENMUC_ID->CurrentValue = NULL;
                                        if (!ew_CompareValue($t_thongbao_levelsite->C_FILEANH->DbValue, $rs->fields('C_FILEANH')))
						$t_thongbao_levelsite->C_FILEANH->CurrentValue = NULL;
				}
				$i++;
				$rs->MoveNext();
			}
	...
	function LoadFormValues() {
		...
		$t_thongbao_levelsite->C_NOTICE_HIT->MultiUpdate = $objForm->GetValue("u_C_NOTICE_HIT");
                $t_thongbao_levelsite->PK_CHUYENMUC_ID->setFormValue($objForm->GetValue("x_PK_CHUYENMUC_ID"));
		$t_thongbao_levelsite->PK_CHUYENMUC_ID->MultiUpdate = $objForm->GetValue("u_PK_CHUYENMUC_ID");
                $t_thongbao_levelsite->C_FILEANH->setFormValue($objForm->GetValue("x_C_FILEANH"));
		$t_thongbao_levelsite->C_FILEANH->MultiUpdate = $objForm->GetValue("u_C_FILEANH");
	...
	function RestoreFormValues() {
		...
		$t_thongbao_levelsite->C_KEYWORD->CurrentValue = $t_thongbao_levelsite->C_KEYWORD->FormValue;
                ...
                $t_thongbao_levelsite->PK_CHUYENMUC_ID->CurrentValue = $t_thongbao_levelsite->PK_CHUYENMUC_ID->FormValue;
                $t_thongbao_levelsite->C_FILEANH->CurrentValue = $t_thongbao_levelsite->C_FILEANH->FormValue;
	...
	function RenderRow() {
		...
		$t_thongbao_levelsite->C_NOTICE_HIT->CellAttrs = array(); $t_thongbao_levelsite->C_NOTICE_HIT->ViewAttrs = array(); $t_thongbao_levelsite->C_NOTICE_HIT->EditAttrs = array();
                ...
                // PK_CHUYENMUC_ID
		$t_thongbao_levelsite->PK_CHUYENMUC_ID->CellCssStyle = ""; $t_thongbao_levelsite->PK_CHUYENMUC_ID->CellCssClass = "";
		$t_thongbao_levelsite->PK_CHUYENMUC_ID->CellAttrs = array(); $t_thongbao_levelsite->PK_CHUYENMUC_ID->ViewAttrs = array(); $t_thongbao_levelsite->PK_CHUYENMUC_ID->EditAttrs = array();
                
		// C_FILEANH
		$t_thongbao_levelsite->C_FILEANH->CellCssStyle = ""; $t_thongbao_levelsite->C_FILEANH->CellCssClass = "";
		$t_thongbao_levelsite->C_FILEANH->CellAttrs = array(); $t_thongbao_levelsite->C_FILEANH->ViewAttrs = array(); $t_thongbao_levelsite->C_FILEANH->EditAttrs = array();
		if ($t_thongbao_levelsite->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$t_thongbao_levelsite->C_TIME_PUBLISH->ViewCustomAttributes = "";
                        ...
                        // PK_CHUYENMUC_ID
			if (strval($t_thongbao_levelsite->PK_CHUYENMUC_ID->CurrentValue) <> "") {
				$sFilterWrk = "`PK_CHUYENMUC` = " . ew_AdjustSql($t_thongbao_levelsite->PK_CHUYENMUC_ID->CurrentValue) . "";
			$sSqlWrk = "SELECT `FK_CONGTY` FROM `t_chuyenmuc_levelsite`";
			$sWhereWrk = "";
			if ($sFilterWrk <> "") {
				if ($sWhereWrk <> "") $sWhereWrk .= " AND ";
				$sWhereWrk .= "(" . $sFilterWrk . ")";
			...
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$t_thongbao_levelsite->PK_CHUYENMUC_ID->ViewValue = $rswrk->fields('FK_CONGTY');
					$rswrk->Close();
				} else {
					$t_thongbao_levelsite->PK_CHUYENMUC_ID->ViewValue = $t_thongbao_levelsite->PK_CHUYENMUC_ID->CurrentValue;
				}
			} else {
				$t_thongbao_levelsite->PK_CHUYENMUC_ID->ViewValue = NULL;
			}
			$t_thongbao_levelsite->PK_CHUYENMUC_ID->CssStyle = "";
			$t_thongbao_levelsite->PK_CHUYENMUC_ID->CssClass = "";
			$t_thongbao_levelsite->PK_CHUYENMUC_ID->ViewCustomAttributes = "";
                        
			// C_FILEANH
			$t_thongbao_levelsite->C_FILEANH->ViewValue = $t_thongbao_levelsite->C_FILEANH->CurrentValue;
			$t_thongbao_levelsite->C_FILEANH->C_FILEANH->ImageAlt = $t_thongbao_levelsite->C_FILEANH->FldAlt;
			$t_thongbao_levelsite->C_FILEANH->CssStyle = "";
			$t_thongbao_levelsite->C_FILEANH->CssClass = "";
			$t_thongbao_levelsite->C_FILEANH->ViewCustomAttributes = "";
			...
			$t_thongbao_levelsite->C_KEYWORD->TooltipValue = "";
                       	...
                       	// C_FILEANH
			if (!ew_Empty($t_thongbao_levelsite->C_FILEANH->CurrentValue)) {
				$t_thongbao_levelsite->C_FILEANH->HrefValue = ((!empty($t_thongbao_levelsite->C_FILEANH->ViewValue)) ? $t_thongbao_levelsite->C_FILEANH->ViewValue : $t_thongbao_levelsite->C_FILEANH->CurrentValue);
				if ($t_thongbao_levelsite->Export <> "") $t_thongbao_levelsite->C_FILEANH->HrefValue = ew_ConvertFullUrl($t_thongbao_levelsite->C_FILEANH->HrefValue);
			} else {
				$t_thongbao_levelsite->C_FILEANH->HrefValue = "";
			}
			$t_thongbao_levelsite->C_FILEANH->TooltipValue = "";
			...
			$t_thongbao_levelsite->C_TITLE->HrefValue = "";
                        ...
                        // PK_CHUYENMUC_ID
			$t_thongbao_levelsite->PK_CHUYENMUC_ID->EditCustomAttributes = "";
				$sFilterWrk = "";
			$sSqlWrk = "SELECT `PK_CHUYENMUC`, `C_NAME`, '' AS Disp2Fld, '' AS SelectFilterFld FROM `t_chuyenmuc_levelsite`";
			$sWhereWrk = "FK_CONGTY =".$Security->CurrentUserOption();
			if ($sFilterWrk <> "") {
				if ($sWhereWrk <> "") $sWhereWrk .= " AND ";
				$sWhereWrk .= "(" . $sFilterWrk . ")";
			}
			...
			$t_thongbao_levelsite->PK_CHUYENMUC_ID->EditValue = $arwrk;
                        
			// C_FILEANH
			$t_thongbao_levelsite->C_FILEANH->EditCustomAttributes = "";
			$t_thongbao_levelsite->C_FILEANH->EditValue = ew_HtmlEncode($t_thongbao_levelsite->C_FILEANH->CurrentValue);
			...
			$t_thongbao_levelsite->C_TIME_PUBLISH->HrefValue = "";
                        		...
                        		// C_FILEANH
			if (!ew_Empty($t_thongbao_levelsite->C_FILEANH->CurrentValue)) {
				$t_thongbao_levelsite->C_FILEANH->HrefValue = ((!empty($t_thongbao_levelsite->C_FILEANH->EditValue)) ? $t_thongbao_levelsite->C_FILEANH->EditValue : $t_thongbao_levelsite->C_FILEANH->CurrentValue);
				if ($t_thongbao_levelsite->Export <> "") $t_thongbao_levelsite->C_FILEANH->HrefValue = ew_ConvertFullUrl($t_thongbao_levelsite->C_FILEANH->HrefValue);
			} else {
				$t_thongbao_levelsite->C_FILEANH->HrefValue = "";
			}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$t_thongbao_levelsite->PK_CHUYENMUC_ID->SetDbValueDef($rsnew, $t_thongbao_levelsite->PK_CHUYENMUC_ID->CurrentValue, NULL, FALSE);
		        if($t_thongbao_levelsite->C_NOTICE_HIT->CurrentValue == 1)
                        {
                        // C_FILEANH
			$t_thongbao_levelsite->C_FILEANH->SetDbValueDef($rsnew, $t_thongbao_levelsite->C_FILEANH->CurrentValue, NULL, FALSE);
                        } else 
                        {
                         // C_FILEANH
			$t_thongbao_levelsite->C_FILEANH->SetDbValueDef($rsnew, NULL, NULL, FALSE);   
                        }  
                        ...
                        IF ($C_SENDMAIL ==1)
                        {
                                ...
                                $noidung = $t_thongbao_levelsite->C_TITLE->CurrentValue;
			...
			if ($bUpdateRow) {
				...
				$Delete_ID=$conn->Execute($sql);
                                 if (strval($t_thongbao_levelsite->C_ACTIVE->CurrentValue) == 1) {
                                    ...
                                    $sql_primary= "INSERT INTO t_thongbao_mainlevel (PK_THONGBAO,FK_CONGTY_ID,FK_DOITUONG_ID,PK_CHUYENMUC_ID,C_PUBLISH,C_TIME_PUBLISH,C_ACTIVE,C_ORDER,C_KEYWORD,C_NOTICE_HIT,C_BEGIN_DATE,C_END_DATE,C_USER_ADD,C_ADD_TIME,C_USER_EDIT,C_EDIT_TIME,FK_NGUOIDUNG_ID,FK_CONGTY_SEND,C_TYPE) 
                                                VALUES ('".$t_thongbao_levelsite->PK_THONGBAO->CurrentValue."','".$Security->CurrentUserOption()."','".$rs->fields('FK_DOITUONG_ID')."','".$t_thongbao_levelsite->PK_CHUYENMUC_ID->CurrentValue."',
