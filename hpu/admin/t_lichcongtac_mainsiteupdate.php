...
// Create page object
$t_lichcongtac_mainsite_update = new ct_lichcongtac_mainsite_update();
$Page =& $t_lichcongtac_mainsite_update;

// Page init
$t_lichcongtac_mainsite_update->Page_Init();

// Page main
$t_lichcongtac_mainsite_update->Page_Main();
?>
...
// Create page object
var t_lichcongtac_mainsite_update = new ew_Page("t_lichcongtac_mainsite_update");

// page properties
t_lichcongtac_mainsite_update.PageID = "update"; // page ID
t_lichcongtac_mainsite_update.FormID = "ft_lichcongtac_mainsiteupdate"; // form ID
var EW_PAGE_ID = t_lichcongtac_mainsite_update.PageID; // for backward compatibility

// extend page with ValidateForm function
t_lichcongtac_mainsite_update.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (uelm && uelm.checked) {
			if (elm && !ew_HasValue(elm))
				return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_lichcongtac_mainsite->C_MINUTES_STAR->FldCaption()) ?>");
		}
		elm = fobj.elements["x" + infix + "_C_STATUS_STAR"];
		uelm = fobj.elements["u" + infix + "_C_STATUS_STAR"];
		if (uelm && uelm.checked) {
			if (elm && !ew_HasValue(elm))
				return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_lichcongtac_mainsite->C_STATUS_STAR->FldCaption()) ?>");
		}
		...
		if (uelm && uelm.checked) {
			if (elm && !ew_HasValue(elm))
				return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_lichcongtac_mainsite->C_MINUTES_END->FldCaption()) ?>");
		}
		elm = fobj.elements["x" + infix + "_C_STATUS_END"];
		uelm = fobj.elements["u" + infix + "_C_STATUS_END"];
		if (uelm && uelm.checked) {
			if (elm && !ew_HasValue(elm))
				return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_lichcongtac_mainsite->C_STATUS_END->FldCaption()) ?>");
		}
		...
		uelm = fobj.elements["u" + infix + "_C_OPTION"];
		if (uelm && uelm.checked) {
			if (elm && !ew_HasValue(elm))
		...
		uelm = fobj.elements["u" + infix + "_C_WHERE"];
		if (uelm && uelm.checked) {
			if (elm && !ew_HasValue(elm))
...
t_lichcongtac_mainsite_update.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_lichcongtac_mainsite_update.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_lichcongtac_mainsite_update.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_lichcongtac_mainsite_update.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<a href="<?php echo $t_lichcongtac_mainsite->getReturnUrl() ?>"><?php echo $Language->Phrase("BackToList") ?></a>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_lichcongtac_mainsite_update->ShowMessage();
?>
<form name="ft_lichcongtac_mainsiteupdate" id="ft_lichcongtac_mainsiteupdate" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return t_lichcongtac_mainsite_update.ValidateForm(this);">
<p>
<input type="hidden" name="t" id="t" value="t_lichcongtac_mainsite">
<input type="hidden" name="a_update" id="a_update" value="U">
<?php for ($i = 0; $i < $t_lichcongtac_mainsite_update->nKeySelected; $i++) { ?>
<input type="hidden" name="k<?php echo $i+1 ?>_key" id="key<?php echo $i+1 ?>" value="<?php echo ew_HtmlEncode($t_lichcongtac_mainsite_update->arRecKeys[$i]) ?>">
<?php } ?>
...
<?php if ($t_lichcongtac_mainsite->FK_CONGTY->Visible) { // FK_CONGTY ?>
	<tr<?php echo $t_lichcongtac_mainsite->FK_CONGTY->RowAttributes ?>>
		<td<?php echo $t_lichcongtac_mainsite->FK_CONGTY->CellAttributes() ?>><?php echo $t_lichcongtac_mainsite->FK_CONGTY->FldCaption() ?></td>
		<td<?php echo $t_lichcongtac_mainsite->FK_CONGTY->CellAttributes() ?>><span id="el_FK_CONGTY">
...
<?php if ($t_lichcongtac_mainsite->FK_SB_ID->Visible) { // FK_SB_ID ?>
	<tr<?php echo $t_lichcongtac_mainsite->FK_SB_ID->RowAttributes ?>>
		<td<?php echo $t_lichcongtac_mainsite->FK_SB_ID->CellAttributes() ?>><?php echo $t_lichcongtac_mainsite->FK_SB_ID->FldCaption() ?></td>
		<td<?php echo $t_lichcongtac_mainsite->FK_SB_ID->CellAttributes() ?>><span id="el_FK_SB_ID">
...
<?php if ($t_lichcongtac_mainsite->C_TITLE->Visible) { // C_TITLE ?>
	<tr<?php echo $t_lichcongtac_mainsite->C_TITLE->RowAttributes ?>>
		<td<?php echo $t_lichcongtac_mainsite->C_TITLE->CellAttributes() ?>><?php echo $t_lichcongtac_mainsite->C_TITLE->FldCaption() ?></td>
		<td<?php echo $t_lichcongtac_mainsite->C_TITLE->CellAttributes() ?>><span id="el_C_TITLE">
<div<?php echo $t_lichcongtac_mainsite->C_TITLE->ViewAttributes() ?>><?php echo $t_lichcongtac_mainsite->C_TITLE->EditValue ?></div><input type="hidden" name="x_C_TITLE" id="x_C_TITLE" value="<?php echo ew_HtmlEncode($t_lichcongtac_mainsite->C_TITLE->CurrentValue) ?>">
...
<?php if ($t_lichcongtac_mainsite->C_DATE_STAR->Visible) { // C_DATE_STAR ?>
	<tr<?php echo $t_lichcongtac_mainsite->C_DATE_STAR->RowAttributes ?>>
		<td<?php echo $t_lichcongtac_mainsite->C_DATE_STAR->CellAttributes() ?>><?php echo $t_lichcongtac_mainsite->C_DATE_STAR->FldCaption() ?></td>
...
Calendar.setup({
 ...
 <span> <?php echo $t_lichcongtac_mainsite->C_HOUR_START->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?>&nbsp;</span>               
...
<?php if ($t_lichcongtac_mainsite->C_STATUS_STAR->Visible) { // C_STATUS_STAR ?>
	<tr<?php echo $t_lichcongtac_mainsite->C_STATUS_STAR->RowAttributes ?>>
		<td<?php echo $t_lichcongtac_mainsite->C_STATUS_STAR->CellAttributes() ?>><?php echo $t_lichcongtac_mainsite->C_STATUS_STAR->FldCaption() ?></td>
		<td<?php echo $t_lichcongtac_mainsite->C_STATUS_STAR->CellAttributes() ?>><span id="el_C_STATUS_STAR">
...
</span><?php echo $t_lichcongtac_mainsite->C_STATUS_STAR->CustomMsg ?></td>
	</tr>
<?php } ?>
        
<?php if ($t_lichcongtac_mainsite->C_DATE_END->Visible) { // C_DATE_END ?>
	<tr<?php echo $t_lichcongtac_mainsite->C_DATE_END->RowAttributes ?>>
		<td<?php echo $t_lichcongtac_mainsite->C_DATE_END->CellAttributes() ?>><?php echo $t_lichcongtac_mainsite->C_DATE_END->FldCaption() ?></td>
...
Calendar.setup({
  ...
  <span> <?php echo $t_lichcongtac_mainsite->C_HOUR_END->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span>                   
...
<?php if ($t_lichcongtac_mainsite->C_STATUS_END->Visible) { // C_STATUS_END ?>
	<tr<?php echo $t_lichcongtac_mainsite->C_STATUS_END->RowAttributes ?>>
		<td<?php echo $t_lichcongtac_mainsite->C_STATUS_END->CellAttributes() ?>><?php echo $t_lichcongtac_mainsite->C_STATUS_END->FldCaption() ?></td>
		<td<?php echo $t_lichcongtac_mainsite->C_STATUS_END->CellAttributes() ?>><span id="el_C_STATUS_END">
...
</span><?php echo $t_lichcongtac_mainsite->C_STATUS_END->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($t_lichcongtac_mainsite->C_PARTICIPANTS_ID->Visible) { // C_PARTICIPANTS_ID ?>
	<tr<?php echo $t_lichcongtac_mainsite->C_PARTICIPANTS_ID->RowAttributes ?>>
		...
		<td<?php echo $t_lichcongtac_mainsite->C_PARTICIPANTS_ID->CellAttributes() ?>><?php echo $t_lichcongtac_mainsite->C_PARTICIPANTS_ID->FldCaption() ?></td>
		<td<?php echo $t_lichcongtac_mainsite->C_PARTICIPANTS_ID->CellAttributes() ?>><span id="el_C_PARTICIPANTS_ID">
<select tabindex="-1" class="x_C_PARTICIPANTS_ID" multiple="multiple" style="width:300px" id="x_C_PARTICIPANTS_ID[]" name="x_C_PARTICIPANTS_ID[]" title="<?php echo $t_lichcongtac_mainsite->C_PARTICIPANTS_ID->FldTitle() ?>" multiple="multiple"<?php echo $t_lichcongtac_mainsite->C_PARTICIPANTS_ID->EditAttributes() ?>>
<?php
...
</span><?php echo $t_lichcongtac_mainsite->C_PARTICIPANTS_ID->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($t_lichcongtac_mainsite->C_OPTION->Visible) { // C_OPTION ?>
	<tr<?php echo $t_lichcongtac_mainsite->C_OPTION->RowAttributes ?>>

		<td<?php echo $t_lichcongtac_mainsite->C_OPTION->CellAttributes() ?>><?php echo $t_lichcongtac_mainsite->C_OPTION->FldCaption() ?></td>
		<td<?php echo $t_lichcongtac_mainsite->C_OPTION->CellAttributes() ?>><span id="el_C_OPTION">
...
<?php if ($t_lichcongtac_mainsite->C_WHERE->Visible) { // C_WHERE ?>
	<tr<?php echo $t_lichcongtac_mainsite->C_WHERE->RowAttributes ?>>
		<td<?php echo $t_lichcongtac_mainsite->C_WHERE->CellAttributes() ?>><?php echo $t_lichcongtac_mainsite->C_WHERE->FldCaption() ?></td>
		<td<?php echo $t_lichcongtac_mainsite->C_WHERE->CellAttributes() ?>><span id="el_C_WHERE">
<input type="text" name="x_C_WHERE" id="x_C_WHERE" title="<?php echo $t_lichcongtac_mainsite->C_WHERE->FldTitle() ?>" size="120" maxlength="255" value="<?php echo $t_lichcongtac_mainsite->C_WHERE->EditValue ?>"<?php echo $t_lichcongtac_mainsite->C_WHERE->EditAttributes() ?>>
</span><?php echo $t_lichcongtac_mainsite->C_WHERE->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($t_lichcongtac_mainsite->C_PUBLISH->Visible) { // C_PUBLISH ?>
	<tr<?php echo $t_lichcongtac_mainsite->C_PUBLISH->RowAttributes ?>>
		<td<?php echo $t_lichcongtac_mainsite->C_PUBLISH->CellAttributes() ?>><?php echo $t_lichcongtac_mainsite->C_PUBLISH->FldCaption() ?></td>
		<td<?php echo $t_lichcongtac_mainsite->C_PUBLISH->CellAttributes() ?>><span id="el_C_PUBLISH">
...
<?php
$t_lichcongtac_mainsite_update->Page_Terminate();
?>
<?php
...
// Page class
//
class ct_lichcongtac_mainsite_update {
	...
	// Page object name
	var $PageObjName = 't_lichcongtac_mainsite_update';
	...
	//
	function ct_lichcongtac_mainsite_update() {
	...
	function Page_Main() {
		...
		if ($this->nKeySelected == 0) {
			...
			if (@$_POST["a_update"] <> "") {
				...
				// Get action
				$t_lichcongtac_mainsite->CurrentAction = $_POST["a_update"];
				...
				if (!$this->ValidateForm()) {
					$t_lichcongtac_mainsite->CurrentAction = "I"; // Form error, reset action
					$this->setMessage($gsFormError);
				}
			}
		}
		if ($this->nKeySelected <= 0)
			$this->Page_Terminate("t_lichcongtac_mainsitelist.php"); // No records selected, return to list
		...
		// Render row
		$t_lichcongtac_mainsite->RowType = EW_ROWTYPE_EDIT; // Render edit
		$this->RenderRow();
	}
	...
	function LoadMultiUpdateValues() {
		global $t_lichcongtac_mainsite;
		$t_lichcongtac_mainsite->CurrentFilter = $this->BuildKeyFilter();
		...
		if ($rs = $this->LoadRecordset()) {
			...
			while (!$rs->EOF) {
				if ($i == 1) {
					...
					if (!ew_CompareValue($t_lichcongtac_mainsite->FK_CONGTY->DbValue, $rs->fields('FK_CONGTY')))
						$t_lichcongtac_mainsite->FK_CONGTY->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->FK_SB_ID->DbValue, $rs->fields('FK_SB_ID')))
						$t_lichcongtac_mainsite->FK_SB_ID->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_TITLE->DbValue, $rs->fields('C_TITLE')))
						$t_lichcongtac_mainsite->C_TITLE->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_DATE_STAR->DbValue, $rs->fields('C_DATE_STAR')))
						$t_lichcongtac_mainsite->C_DATE_STAR->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_HOUR_START->DbValue, $rs->fields('C_HOUR_START')))
						$t_lichcongtac_mainsite->C_HOUR_START->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_MINUTES_STAR->DbValue, $rs->fields('C_MINUTES_STAR')))
						$t_lichcongtac_mainsite->C_MINUTES_STAR->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_STATUS_STAR->DbValue, $rs->fields('C_STATUS_STAR')))
						$t_lichcongtac_mainsite->C_STATUS_STAR->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_DATE_END->DbValue, $rs->fields('C_DATE_END')))
						$t_lichcongtac_mainsite->C_DATE_END->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_HOUR_END->DbValue, $rs->fields('C_HOUR_END')))
						$t_lichcongtac_mainsite->C_HOUR_END->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_MINUTES_END->DbValue, $rs->fields('C_MINUTES_END')))
						$t_lichcongtac_mainsite->C_MINUTES_END->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_STATUS_END->DbValue, $rs->fields('C_STATUS_END')))
						$t_lichcongtac_mainsite->C_STATUS_END->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_PARTICIPANTS_ID->DbValue, $rs->fields('C_PARTICIPANTS_ID')))
						$t_lichcongtac_mainsite->C_PARTICIPANTS_ID->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_OPTION->DbValue, $rs->fields('C_OPTION')))
						$t_lichcongtac_mainsite->C_OPTION->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_WHERE->DbValue, $rs->fields('C_WHERE')))
						$t_lichcongtac_mainsite->C_WHERE->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_PUBLISH->DbValue, $rs->fields('C_PUBLISH')))
						$t_lichcongtac_mainsite->C_PUBLISH->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_DATETIME_PUBLISH->DbValue, $rs->fields('C_DATETIME_PUBLISH')))
						$t_lichcongtac_mainsite->C_DATETIME_PUBLISH->CurrentValue = NULL;
					if (!ew_CompareValue($t_lichcongtac_mainsite->C_FK_PUBLISH->DbValue, $rs->fields('C_FK_PUBLISH')))
						$t_lichcongtac_mainsite->C_FK_PUBLISH->CurrentValue = NULL;
				}
				$i++;
				$rs->MoveNext();
			}
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$t_lichcongtac_mainsite->C_CADENLAR_ID->CurrentValue = $sKeyFld;
		return TRUE;
	}

	// Update all selected rows
	function UpdateRows() {
		global $conn, $Language, $t_lichcongtac_mainsite;
		$conn->BeginTrans();
		...
		foreach ($this->arRecKeys as $sThisKey) {
			...
			if ($this->SetupKeyValues($sThisKey)) {
				$t_lichcongtac_mainsite->SendEmail = FALSE; // Do not send email on update success
	...
	function LoadFormValues() {
		...
		$t_lichcongtac_mainsite->FK_CONGTY->MultiUpdate = $objForm->GetValue("u_FK_CONGTY");
		$t_lichcongtac_mainsite->FK_SB_ID->setFormValue($objForm->GetValue("x_FK_SB_ID"));
		$t_lichcongtac_mainsite->FK_SB_ID->MultiUpdate = $objForm->GetValue("u_FK_SB_ID");
		$t_lichcongtac_mainsite->C_TITLE->setFormValue($objForm->GetValue("x_C_TITLE"));
		$t_lichcongtac_mainsite->C_TITLE->MultiUpdate = $objForm->GetValue("u_C_TITLE");
		...
		$t_lichcongtac_mainsite->C_DATE_STAR->MultiUpdate = $objForm->GetValue("u_C_DATE_STAR");
		$t_lichcongtac_mainsite->C_HOUR_START->setFormValue($objForm->GetValue("x_C_HOUR_START"));
		$t_lichcongtac_mainsite->C_HOUR_START->MultiUpdate = $objForm->GetValue("u_C_HOUR_START");
		$t_lichcongtac_mainsite->C_MINUTES_STAR->setFormValue($objForm->GetValue("x_C_MINUTES_STAR"));
		$t_lichcongtac_mainsite->C_MINUTES_STAR->MultiUpdate = $objForm->GetValue("u_C_MINUTES_STAR");
		$t_lichcongtac_mainsite->C_STATUS_STAR->setFormValue($objForm->GetValue("x_C_STATUS_STAR"));
		$t_lichcongtac_mainsite->C_STATUS_STAR->MultiUpdate = $objForm->GetValue("u_C_STATUS_STAR");
		...
		$t_lichcongtac_mainsite->C_DATE_END->MultiUpdate = $objForm->GetValue("u_C_DATE_END");
		$t_lichcongtac_mainsite->C_HOUR_END->setFormValue($objForm->GetValue("x_C_HOUR_END"));
		$t_lichcongtac_mainsite->C_HOUR_END->MultiUpdate = $objForm->GetValue("u_C_HOUR_END");
		$t_lichcongtac_mainsite->C_MINUTES_END->setFormValue($objForm->GetValue("x_C_MINUTES_END"));
		$t_lichcongtac_mainsite->C_MINUTES_END->MultiUpdate = $objForm->GetValue("u_C_MINUTES_END");
		$t_lichcongtac_mainsite->C_STATUS_END->setFormValue($objForm->GetValue("x_C_STATUS_END"));
		$t_lichcongtac_mainsite->C_STATUS_END->MultiUpdate = $objForm->GetValue("u_C_STATUS_END");
		$t_lichcongtac_mainsite->C_PARTICIPANTS_ID->setFormValue($objForm->GetValue("x_C_PARTICIPANTS_ID"));
		$t_lichcongtac_mainsite->C_PARTICIPANTS_ID->MultiUpdate = $objForm->GetValue("u_C_PARTICIPANTS_ID");
		$t_lichcongtac_mainsite->C_OPTION->setFormValue($objForm->GetValue("x_C_OPTION"));
		$t_lichcongtac_mainsite->C_OPTION->MultiUpdate = $objForm->GetValue("u_C_OPTION");
		$t_lichcongtac_mainsite->C_WHERE->setFormValue($objForm->GetValue("x_C_WHERE"));
		$t_lichcongtac_mainsite->C_WHERE->MultiUpdate = $objForm->GetValue("u_C_WHERE");
		$t_lichcongtac_mainsite->C_PUBLISH->setFormValue($objForm->GetValue("x_C_PUBLISH"));
		$t_lichcongtac_mainsite->C_PUBLISH->MultiUpdate = $objForm->GetValue("u_C_PUBLISH");
		...
		$t_lichcongtac_mainsite->C_DATETIME_PUBLISH->MultiUpdate = $objForm->GetValue("u_C_DATETIME_PUBLISH");
		$t_lichcongtac_mainsite->C_FK_PUBLISH->setFormValue($objForm->GetValue("x_C_FK_PUBLISH"));
		$t_lichcongtac_mainsite->C_FK_PUBLISH->MultiUpdate = $objForm->GetValue("u_C_FK_PUBLISH");
	...
	function RenderRow() {
		...
		if ($t_lichcongtac_mainsite->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$t_lichcongtac_mainsite->C_TITLE->EditValue = $t_lichcongtac_mainsite->C_TITLE->CurrentValue;
			$t_lichcongtac_mainsite->C_TITLE->CssStyle = "";
			$t_lichcongtac_mainsite->C_TITLE->CssClass = "";
			$t_lichcongtac_mainsite->C_TITLE->ViewCustomAttributes = "";
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($t_lichcongtac_mainsite->C_DATE_STAR->MultiUpdate <> "" && !is_null($t_lichcongtac_mainsite->C_DATE_STAR->FormValue) && $t_lichcongtac_mainsite->C_DATE_STAR->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac_mainsite->C_DATE_STAR->FldCaption();
		}
		if ($t_lichcongtac_mainsite->C_DATE_STAR->MultiUpdate <> "") {
		...
		if ($t_lichcongtac_mainsite->C_HOUR_START->MultiUpdate <> "" && !is_null($t_lichcongtac_mainsite->C_HOUR_START->FormValue) && $t_lichcongtac_mainsite->C_HOUR_START->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac_mainsite->C_HOUR_START->FldCaption();
		}
		if ($t_lichcongtac_mainsite->C_MINUTES_STAR->MultiUpdate <> "" && !is_null($t_lichcongtac_mainsite->C_MINUTES_STAR->FormValue) && $t_lichcongtac_mainsite->C_MINUTES_STAR->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac_mainsite->C_MINUTES_STAR->FldCaption();
		}
		if ($t_lichcongtac_mainsite->C_STATUS_STAR->MultiUpdate <> "" && $t_lichcongtac_mainsite->C_STATUS_STAR->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac_mainsite->C_STATUS_STAR->FldCaption();
		}
		if ($t_lichcongtac_mainsite->C_DATE_END->MultiUpdate <> "" && !is_null($t_lichcongtac_mainsite->C_DATE_END->FormValue) && $t_lichcongtac_mainsite->C_DATE_END->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac_mainsite->C_DATE_END->FldCaption();
		}
		if ($t_lichcongtac_mainsite->C_DATE_END->MultiUpdate <> "") {
			if (!ew_CheckEuroDate($t_lichcongtac_mainsite->C_DATE_END->FormValue)) {
				if ($gsFormError <> "") $gsFormError .= "<br>";
				$gsFormError .= $t_lichcongtac_mainsite->C_DATE_END->FldErrMsg;
			}
		}
		if ($t_lichcongtac_mainsite->C_HOUR_END->MultiUpdate <> "" && !is_null($t_lichcongtac_mainsite->C_HOUR_END->FormValue) && $t_lichcongtac_mainsite->C_HOUR_END->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac_mainsite->C_HOUR_END->FldCaption();
		}
		if ($t_lichcongtac_mainsite->C_MINUTES_END->MultiUpdate <> "" && !is_null($t_lichcongtac_mainsite->C_MINUTES_END->FormValue) && $t_lichcongtac_mainsite->C_MINUTES_END->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac_mainsite->C_MINUTES_END->FldCaption();
		}
		if ($t_lichcongtac_mainsite->C_STATUS_END->MultiUpdate <> "" && $t_lichcongtac_mainsite->C_STATUS_END->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac_mainsite->C_STATUS_END->FldCaption();
		}
		if ($t_lichcongtac_mainsite->C_PARTICIPANTS_ID->MultiUpdate <> "" && $t_lichcongtac_mainsite->C_PARTICIPANTS_ID->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac_mainsite->C_PARTICIPANTS_ID->FldCaption();
		}
		if ($t_lichcongtac_mainsite->C_OPTION->MultiUpdate <> "" && $t_lichcongtac_mainsite->C_OPTION->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac_mainsite->C_OPTION->FldCaption();
		}
		if ($t_lichcongtac_mainsite->C_WHERE->MultiUpdate <> "" && !is_null($t_lichcongtac_mainsite->C_WHERE->FormValue) && $t_lichcongtac_mainsite->C_WHERE->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac_mainsite->C_WHERE->FldCaption();
		}
		if ($t_lichcongtac_mainsite->C_PUBLISH->MultiUpdate <> "" && $t_lichcongtac_mainsite->C_PUBLISH->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$t_lichcongtac_mainsite->C_PUBLISH->SetDbValueDef($rsnew, $t_lichcongtac_mainsite->C_PUBLISH->CurrentValue, NULL, FALSE);
                         ...
                         if ($t_lichcongtac_mainsite->C_PUBLISH->CurrentValue ==2)
                         {
                             // C_DATETIME_PUBLISH		
