...
// Create page object
$t_linkad_edit = new ct_linkad_edit();
$Page =& $t_linkad_edit;

// Page init
$t_linkad_edit->Page_Init();

// Page main
$t_linkad_edit->Page_Main();
?>
...
<p><span class="phpmaker"><a href="<?php echo $t_linkad->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_linkad_edit->ShowMessage();
?>
<form name="ft_linkadedit" id="ft_linkadedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return t_linkad_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="t_linkad">
...
<div<?php echo $t_linkad->FK_CONGTY->ViewAttributes() ?>><?php echo $t_linkad->FK_CONGTY->EditValue ?></div><input type="hidden" name="x_FK_CONGTY" id="x_FK_CONGTY" value="<?php echo ew_HtmlEncode($t_linkad->FK_CONGTY->CurrentValue) ?>">
...
<textarea name="x_C_LINKAD_DES" id="x_C_LINKAD_DES" title="<?php echo $t_linkad->C_LINKAD_DES->FldTitle() ?>" cols="67" rows="1" <?php echo $t_linkad->C_LINKAD_DES->EditAttributes() ?>><?php echo $t_linkad->C_LINKAD_DES->EditValue ?></textarea>
...
<input type="hidden" name="x_C_LINKAD_ID" id="x_C_LINKAD_ID" value="<?php echo ew_HtmlEncode($t_linkad->C_LINKAD_ID->CurrentValue) ?>">
<p>
...
class ct_linkad_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$t_linkad->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$t_linkad->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$t_linkad->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$t_linkad->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($t_linkad->C_LINKAD_ID->CurrentValue == "")
			$this->Page_Terminate("t_linkadlist.php"); // Invalid key, return to list
		switch ($t_linkad->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$t_linkad->SendEmail = TRUE; // Send email on update success
				if ($this->EditRow()) { // Update record based on key
					$this->setMessage($Language->Phrase("UpdateSuccess")); // Update success
					$sReturnUrl = $t_linkad->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "t_linkadview.php")
						$sReturnUrl = $t_linkad->ViewUrl(); // View paging, return to View page directly
		...
		// Render the record
		$t_linkad->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		if ($t_linkad->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($t_linkad->RowType == EW_ROWTYPE_EDIT) { // Edit row
			...
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$t_linkad->FK_CONGTY->EditValue = $rswrk->fields('C_TENCONGTY');
					$rswrk->Close();
				} else {
					$t_linkad->FK_CONGTY->EditValue = $t_linkad->FK_CONGTY->CurrentValue;
				}
			} else {
				$t_linkad->FK_CONGTY->EditValue = NULL;
			}
			$t_linkad->FK_CONGTY->CssStyle = "";
			$t_linkad->FK_CONGTY->CssClass = "";
			$t_linkad->FK_CONGTY->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $t_linkad->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($t_linkad->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$t_linkad->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
