...
$t_doituong_cuusv = NULL;

//
// Table class for t_doituong_cuusv
//
class ct_doituong_cuusv {
	var $TableVar = 't_doituong_cuusv';
	...
	//
	function ct_doituong_cuusv() {
		global $Language;

		// DTCUUSV_ID
		$this->DTCUUSV_ID = new cField('t_doituong_cuusv', 't_doituong_cuusv', 'x_DTCUUSV_ID', 'DTCUUSV_ID', '`DTCUUSV_ID`', 3, -1, FALSE, '`DTCUUSV_ID`', FALSE);
		$this->DTCUUSV_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['DTCUUSV_ID'] =& $this->DTCUUSV_ID;

		// C_NAME
		$this->C_NAME = new cField('t_doituong_cuusv', 't_doituong_cuusv', 'x_C_NAME', 'C_NAME', '`C_NAME`', 200, -1, FALSE, '`C_NAME`', FALSE);
		$this->fields['C_NAME'] =& $this->C_NAME;

		// C_BELONGTO
		$this->C_BELONGTO = new cField('t_doituong_cuusv', 't_doituong_cuusv', 'x_C_BELONGTO', 'C_BELONGTO', '`C_BELONGTO`', 3, -1, FALSE, '`C_BELONGTO`', FALSE);
		$this->C_BELONGTO->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['C_BELONGTO'] =& $this->C_BELONGTO;

		// C_TYPE
		$this->C_TYPE = new cField('t_doituong_cuusv', 't_doituong_cuusv', 'x_C_TYPE', 'C_TYPE', '`C_TYPE`', 3, -1, FALSE, '`C_TYPE`', FALSE);
		$this->C_TYPE->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['C_TYPE'] =& $this->C_TYPE;

		// C_URL
		$this->C_URL = new cField('t_doituong_cuusv', 't_doituong_cuusv', 'x_C_URL', 'C_URL', '`C_URL`', 200, -1, FALSE, '`C_URL`', FALSE);
		$this->fields['C_URL'] =& $this->C_URL;

		// C_ACTIVE
		$this->C_ACTIVE = new cField('t_doituong_cuusv', 't_doituong_cuusv', 'x_C_ACTIVE', 'C_ACTIVE', '`C_ACTIVE`', 3, -1, FALSE, '`C_ACTIVE`', FALSE);
		$this->C_ACTIVE->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['C_ACTIVE'] =& $this->C_ACTIVE;

		// C_USER_ADD
		$this->C_USER_ADD = new cField('t_doituong_cuusv', 't_doituong_cuusv', 'x_C_USER_ADD', 'C_USER_ADD', '`C_USER_ADD`', 3, -1, FALSE, '`C_USER_ADD`', FALSE);
		$this->C_USER_ADD->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['C_USER_ADD'] =& $this->C_USER_ADD;

		// C_TIME_ADD
		$this->C_TIME_ADD = new cField('t_doituong_cuusv', 't_doituong_cuusv', 'x_C_TIME_ADD', 'C_TIME_ADD', '`C_TIME_ADD`', 135, 7, FALSE, '`C_TIME_ADD`', FALSE);
		$this->C_TIME_ADD->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['C_TIME_ADD'] =& $this->C_TIME_ADD;

		// C_USER_EDIT
		$this->C_USER_EDIT = new cField('t_doituong_cuusv', 't_doituong_cuusv', 'x_C_USER_EDIT', 'C_USER_EDIT', '`C_USER_EDIT`', 3, -1, FALSE, '`C_USER_EDIT`', FALSE);
		$this->C_USER_EDIT->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['C_USER_EDIT'] =& $this->C_USER_EDIT;

		// C_TIME_EDIT
		$this->C_TIME_EDIT = new cField('t_doituong_cuusv', 't_doituong_cuusv', 'x_C_TIME_EDIT', 'C_TIME_EDIT', '`C_TIME_EDIT`', 135, 7, FALSE, '`C_TIME_EDIT`', FALSE);
		$this->C_TIME_EDIT->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['C_TIME_EDIT'] =& $this->C_TIME_EDIT;

		// FK_CONGTY
		$this->FK_CONGTY = new cField('t_doituong_cuusv', 't_doituong_cuusv', 'x_FK_CONGTY', 'FK_CONGTY', '`FK_CONGTY`', 3, -1, FALSE, '`FK_CONGTY`', FALSE);
		$this->FK_CONGTY->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['FK_CONGTY'] =& $this->FK_CONGTY;
                
                // C_ORDER
		$this->C_ORDER = new cField('t_doituong_cuusv', 't_doituong_cuusv', 'x_C_ORDER', 'C_ORDER', '`C_ORDER`', 3, -1, FALSE, '`C_ORDER`', FALSE);
	...
	function HighlightName() {
		return "t_doituong_cuusv_Highlight";
	}
	...
	function SqlWhere() { // Where
		$sWhere = "";
                if ( $_SESSION['CSV_BELONGTO_ID'] == null)
                            {  $sWhere = "`t_doituong_cuusv`.`C_BELONGTO` = 0";  }
                             else { $sWhere = "`t_doituong_cuusv`.`C_BELONGTO` = ".@$_SESSION['CSV_BELONGTO_ID']; }
		$this->TableFilter = "";
		if ($this->TableFilter <> "") {
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `t_doituong_cuusv` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `t_doituong_cuusv` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `t_doituong_cuusv` WHERE ";
		$SQL .= ew_QuotedName('DTCUUSV_ID') . '=' . ew_QuotedValue($rs['DTCUUSV_ID'], $this->DTCUUSV_ID->FldDataType) . ' AND ';
	...
	function SqlKeyFilter() {
		return "`DTCUUSV_ID` = @DTCUUSV_ID@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->DTCUUSV_ID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@DTCUUSV_ID@", ew_AdjustSql($this->DTCUUSV_ID->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "t_doituong_cuusvlist.php";
		}
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("t_doituong_cuusvview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "t_doituong_cuusvadd.php";
	...
	// Edit URL
	function EditUrl() {
		return $this->KeyUrl("t_doituong_cuusvedit.php", $this->UrlParm());
	}
	...
	// Copy URL
	function CopyUrl() {
		return $this->KeyUrl("t_doituong_cuusvadd.php", $this->UrlParm());
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("t_doituong_cuusvdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->DTCUUSV_ID->CurrentValue)) {
			$sUrl .= "DTCUUSV_ID=" . urlencode($this->DTCUUSV_ID->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=t_doituong_cuusv" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function LoadListRowValues(&$rs) {
		$this->DTCUUSV_ID->setDbValue($rs->fields('DTCUUSV_ID'));
		...
		if (strval($this->C_TYPE->CurrentValue) <> "") {
			switch ($this->C_TYPE->CurrentValue) {
				...
				case "1":
					...
					break;
				case "2":
					$this->C_TYPE->ViewValue = "Danh m?c url lien ket";
					break;
				default:
