...
t_thongbao_mainlevel_ob_edit.FormID = "ft_thongbao_mainlevel_obedit"; // form ID
...
t_thongbao_mainlevel_ob_edit.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_thongbao_mainlevel_ob->FK_DOITUONG_ID->FldCaption()) ?>");
...
?>
<form name="ft_thongbao_mainlevel_obedit" id="ft_thongbao_mainlevel_obedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return t_thongbao_mainlevel_ob_edit.ValidateForm(this);">
<p>
...
class ct_thongbao_mainlevel_ob_edit {
	...
	function RenderRow() {
		...
		if ($t_thongbao_mainlevel_ob->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$arwrk = array();
			$arwrk[] = array("0", "Khoong kích hoạt");
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// C_NOTICE_HIT
			$t_thongbao_mainlevel_ob->C_NOTICE_HIT->SetDbValueDef($rsnew, ew_CurrentDate(), NULL);
			$rsnew['C_NOTICE_HIT'] =& $t_thongbao_mainlevel_ob->C_NOTICE_HIT->DbValue;
