...
class cLanguage {
	...
	function LoadFileList() {
		...
		if (is_array($EW_LANGUAGE_FILE)) {
			...
			for ($i = 0; $i < $cnt; $i++)
				$EW_LANGUAGE_FILE[$i][1] = $this->LoadFileDesc("../admin/" .EW_LANGUAGE_FOLDER . $EW_LANGUAGE_FILE[$i][2]);
		}
	}
		...
		if (is_array($EW_LANGUAGE_FILE)) {
			...
			for ($i = 0; $i < $cnt; $i++)
				if ($EW_LANGUAGE_FILE[$i][0] == $Id) {
					return "../admin/" . EW_LANGUAGE_FOLDER . $EW_LANGUAGE_FILE[$i][2];
			}
		}
		return "";
	}
 ...
 */
Class cMenuCSS {
	function AddMenuItem($text,$url,$allowed =true) {
		if ($allowed == true){
                    echo "<li><a href=\"" . $url . "\" class=\"fly\">". $text . "</a></li>";
		}
	}
}
// Debug timer
...
//TuanDA
function AllowListMenuex($TableName) {
	...
	if (IsLoggedIn()) {
		if (IsAdmin()) {
			...
			$priv = 0;
			if (is_array(@$_SESSION[EW_SESSION_AR_SUB_USER_LEVEL_PRIV])) {
				foreach ($_SESSION[EW_SESSION_AR_SUB_USER_LEVEL_PRIV] as $row) {
                                      if (strval($row[0]) == strval($TableName) &&
						$row[1] == $userid) {
						$thispriv = $row[2];
