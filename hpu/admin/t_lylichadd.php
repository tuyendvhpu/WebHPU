...
// Create page object
$t_lylich_add = new ct_lylich_add();
$Page =& $t_lylich_add;

// Page init
$t_lylich_add->Page_Init();

// Page main
$t_lylich_add->Page_Main();
?>
...
// Create page object
var t_lylich_add = new ew_Page("t_lylich_add");

// page properties
t_lylich_add.PageID = "add"; // page ID
t_lylich_add.FormID = "ft_lylichadd"; // form ID
var EW_PAGE_ID = t_lylich_add.PageID; // for backward compatibility

// extend page with ValidateForm function
t_lylich_add.ValidateForm = function(fobj) {
...
t_lylich_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_lylich_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_lylich_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_lylich_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_lylich_add->ShowMessage();
?>
<form name="ft_lylichadd" id="ft_lylichadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return t_lylich_add.ValidateForm(this);">
<p>
...
class ct_lylich_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $t_lylich->CurrentAction = $_POST["a_add"]; // Get form action
		  ...
		  if ($bCopy) {
		    $t_lylich->CurrentAction = "C"; // Copy record
		  } else {
		    $t_lylich->CurrentAction = "I"; // Display blank record
		...
		switch ($t_lylich->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$t_lylich->SendEmail = TRUE; // Send email on add success
		    if ($this->AddRow()) { // Add successful
		      $this->setMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $t_lylich->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "t_lylichview.php")
						$sReturnUrl = $t_lylich->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$t_lylich->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $t_lylich;
		$t_lylich->C_PIC->CurrentValue = NULL; // Clear file related field
		$t_lylich->C_TEMPLATE->CurrentValue = 1;
		$t_lylich->C_STATUS->CurrentValue = 1;
	}
	...
	function LoadFormValues() {
		...
		$t_lylich->C_USER_ADD->setFormValue($objForm->GetValue("x_C_USER_ADD"));
		$t_lylich->C_ADD_TIME->setFormValue($objForm->GetValue("x_C_ADD_TIME"));
		$t_lylich->C_ADD_TIME->CurrentValue = ew_UnFormatDateTime($t_lylich->C_ADD_TIME->CurrentValue, 7);
	...
	function RestoreFormValues() {
		...
		$t_lylich->C_USER_ADD->CurrentValue = $t_lylich->C_USER_ADD->FormValue;
		$t_lylich->C_ADD_TIME->CurrentValue = $t_lylich->C_ADD_TIME->FormValue;
		$t_lylich->C_ADD_TIME->CurrentValue = ew_UnFormatDateTime($t_lylich->C_ADD_TIME->CurrentValue, 7);
	}
	...
	function RenderRow() {
		...
		} elseif ($t_lylich->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		// C_USER_ADD
		$t_lylich->C_USER_ADD->SetDbValueDef($rsnew, CurrentUserID(), NULL);
		$rsnew['C_USER_ADD'] =& $t_lylich->C_USER_ADD->DbValue;

		// C_ADD_TIME
		$t_lylich->C_ADD_TIME->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
		$rsnew['C_ADD_TIME'] =& $t_lylich->C_ADD_TIME->DbValue;
                
                // C_ACTIVE
		$t_lylich->C_ACTIVE->SetDbValueDef($rsnew, 2, NULL, FALSE);

		// Call Row Inserting event
		$bInsertRow = $t_lylich->Row_Inserting($rsnew);
		if ($bInsertRow) {
			if (!ew_Empty($t_lylich->C_PIC->Upload->Value)) {
				$t_lylich->C_PIC->Upload->SaveToFile($t_lylich->C_PIC->UploadPath, $rsnew['C_PIC'], FALSE);
			}
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($t_lylich->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$t_lylich->PK_PROFILE_ID->setDbValue($conn->Insert_ID());
			$rsnew['PK_PROFILE_ID'] = $t_lylich->PK_PROFILE_ID->DbValue;

			// Call Row Inserted event
			$t_lylich->Row_Inserted($rsnew);
		}
