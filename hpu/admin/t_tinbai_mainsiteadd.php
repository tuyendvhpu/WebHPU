...
// Create page object
$t_tinbai_mainsite_add = new ct_tinbai_mainsite_add();
$Page =& $t_tinbai_mainsite_add;

// Page init
$t_tinbai_mainsite_add->Page_Init();

// Page main
$t_tinbai_mainsite_add->Page_Main();
?>
...
// Create page object
var t_tinbai_mainsite_add = new ew_Page("t_tinbai_mainsite_add");

// page properties
t_tinbai_mainsite_add.PageID = "add"; // page ID
t_tinbai_mainsite_add.FormID = "ft_tinbai_mainsiteadd"; // form ID
var EW_PAGE_ID = t_tinbai_mainsite_add.PageID; // for backward compatibility

// extend page with ValidateForm function
t_tinbai_mainsite_add.ValidateForm = function(fobj) {
...
t_tinbai_mainsite_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_tinbai_mainsite_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_tinbai_mainsite_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_tinbai_mainsite_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<script src="js/select2.js"></script>
    ...
    <script id="script_e14">
        $(document).ready(function () {
            $(".x_FK_ARRAY_CONGTY").val(["AL","AZ"]).select2();
...
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_tinbai_mainsite_add->ShowMessage();
?>
<form name="ft_tinbai_mainsiteadd" id="ft_tinbai_mainsiteadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return t_tinbai_mainsite_add.ValidateForm(this);">
<p>
...
</span><?php echo $t_tinbai_mainsite->FK_ARRAY_CONGTY->CustomMsg ?>
                    <i>(*Click hoặc gõ để chọn nhiều đơn vị)</i>               
                </td>
	</tr>
<?php } ?>
...
<input type="text" name="x_C_PIC_MYSEFLT" id="x_C_PIC_MYSEFLT" title="<?php echo $t_tinbai_mainsite->C_PIC_MYSEFLT->FldTitle() ?>" size="80" maxlength="255" value="<?php echo $t_tinbai_mainsite->C_PIC_MYSEFLT->EditValue ?>"<?php echo $t_tinbai_mainsite->C_PIC_MYSEFLT->EditAttributes() ?>>
                ...
                (Ảnh tin chúng tôi hiển thị tốt nhất: 330px* 211px)--<a target="_blank" href="http://img.hpu.edu.vn"> Kho ảnh sự kiện HPU </a>
                </td>
	</tr>
<?php } ?>
...
<?php
$t_tinbai_mainsite_add->Page_Terminate();
?>
<?php
...
class ct_tinbai_mainsite_add {
	...
	// Page object name
	var $PageObjName = 't_tinbai_mainsite_add';
	...
	//
	function ct_tinbai_mainsite_add() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $t_tinbai_mainsite->CurrentAction = $_POST["a_add"]; // Get form action
		  ...
		  if ($bCopy) {
		    $t_tinbai_mainsite->CurrentAction = "C"; // Copy record
		  } else {
		    $t_tinbai_mainsite->CurrentAction = "I"; // Display blank record
		...
		switch ($t_tinbai_mainsite->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$t_tinbai_mainsite->SendEmail = TRUE; // Send email on add success
		    if ($this->AddRow()) { // Add successful
		      $this->setMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $t_tinbai_mainsite->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "t_tinbai_mainsiteview.php")
						$sReturnUrl = $t_tinbai_mainsite->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$t_tinbai_mainsite->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $t_tinbai_mainsite;
		$t_tinbai_mainsite->C_HIT_MAINSITE->CurrentValue = 1;
		$t_tinbai_mainsite->C_PIC_HIT->CurrentValue = NULL; // Clear file related field
		$t_tinbai_mainsite->C_NEW_MYSEFLT->CurrentValue = 1;
		$t_tinbai_mainsite->C_PIC_MYSEFLT->CurrentValue = NULL; // Clear file related field
		$t_tinbai_mainsite->C_COMMENT_MAINSITE->CurrentValue = 0;
                $t_tinbai_mainsite->C_SENDMAIL->CurrentValue = 0;
	}
	...
	function LoadFormValues() {
		...
		$t_tinbai_mainsite->C_USER_ADD->setFormValue($objForm->GetValue("x_C_USER_ADD"));
		$t_tinbai_mainsite->C_ADD_TIME->setFormValue($objForm->GetValue("x_C_ADD_TIME"));
		$t_tinbai_mainsite->C_ADD_TIME->CurrentValue = ew_UnFormatDateTime($t_tinbai_mainsite->C_ADD_TIME->CurrentValue, 7);
		$t_tinbai_mainsite->FK_EDITOR_ID->setFormValue($objForm->GetValue("x_FK_EDITOR_ID"));
	...
	function RestoreFormValues() {
		...
		$t_tinbai_mainsite->C_USER_ADD->CurrentValue = $t_tinbai_mainsite->C_USER_ADD->FormValue;
		$t_tinbai_mainsite->C_ADD_TIME->CurrentValue = $t_tinbai_mainsite->C_ADD_TIME->FormValue;
		$t_tinbai_mainsite->C_ADD_TIME->CurrentValue = ew_UnFormatDateTime($t_tinbai_mainsite->C_ADD_TIME->CurrentValue, 7);
		$t_tinbai_mainsite->FK_EDITOR_ID->CurrentValue = $t_tinbai_mainsite->FK_EDITOR_ID->FormValue;
	...
	function RenderRow() {
		...
		$t_tinbai_mainsite->C_PIC_LIB->CellAttrs = array(); $t_tinbai_mainsite->C_PIC_LIB->ViewAttrs = array(); $t_tinbai_mainsite->C_PIC_LIB->EditAttrs = array();
		...
		} elseif ($t_tinbai_mainsite->RowType == EW_ROWTYPE_ADD) { // Add row
			...
			$arwrk = array();
			$arwrk[] = array("0", "Không là tin chúng tối");
	...
	function AddRow() {
		...
		// C_USER_ADD
		$t_tinbai_mainsite->C_USER_ADD->SetDbValueDef($rsnew, CurrentUserID(), NULL);
		$rsnew['C_USER_ADD'] =& $t_tinbai_mainsite->C_USER_ADD->DbValue;

		// C_ADD_TIME
		$t_tinbai_mainsite->C_ADD_TIME->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
		$rsnew['C_ADD_TIME'] =& $t_tinbai_mainsite->C_ADD_TIME->DbValue;

		// FK_EDITOR_ID
		$t_tinbai_mainsite->FK_EDITOR_ID->SetDbValueDef($rsnew, -20, NULL);
		$rsnew['FK_EDITOR_ID'] =& $t_tinbai_mainsite->FK_EDITOR_ID->DbValue;
                
                // C_ACTIVE_MAINSITE
		$t_tinbai_mainsite->C_ACTIVE_MAINSITE->SetDbValueDef($rsnew, 0, NULL);
		$rsnew['C_ACTIVE_MAINSITE'] =& $t_tinbai_mainsite->C_ACTIVE_MAINSITE->DbValue;
                
                // C_SENDMAIL
		$t_tinbai_mainsite->C_SENDMAIL->SetDbValueDef($rsnew, $t_tinbai_mainsite->C_SENDMAIL->CurrentValue, NULL, FALSE);

		if ($t_tinbai_mainsite->C_SENDMAIL->CurrentValue == '1')
                {   
		// C_CONTENTS_MAIL  
		$t_tinbai_mainsite->C_CONTENTS_MAIL->SetDbValueDef($rsnew, $t_tinbai_mainsite->C_CONTENTS_MAIL->CurrentValue, NULL, FALSE);
                // send Mail 
                                        ...
                                        $noidung = $t_tinbai_mainsite->C_TITLE->CurrentValue;
                                        $hinhanh = $t_tinbai_mainsite->C_CONTENTS_MAIL->CurrentValue;
                ...
                {
                    $t_tinbai_mainsite->C_CONTENTS_MAIL->SetDbValueDef($rsnew, NULL, NULL, FALSE);
                } 
		...
		$bInsertRow = $t_tinbai_mainsite->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($t_tinbai_mainsite->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$t_tinbai_mainsite->PK_TINBAI_ID->setDbValue($conn->Insert_ID());
			$rsnew['PK_TINBAI_ID'] = $t_tinbai_mainsite->PK_TINBAI_ID->DbValue;
                      //ECHO $t_tinbai_mainsite->PK_TINBAI_ID->DbValue;
                     // ECHO   $t_tinbai_mainsite->FK_ARRAY_CONGTY->CurrentValue;
             
                      if (strval($t_tinbai_mainsite->FK_ARRAY_CONGTY->CurrentValue) <> "") {
				$arwrk = explode(",", $t_tinbai_mainsite->FK_ARRAY_CONGTY->CurrentValue);
				foreach ($arwrk as $wrk) {
                                        $sql = "INSERT INTO t_tinbai_mainlevel (PK_TINBAI_ID,FK_CONGTY,FK_EDITOR_ID) VALUES ( '".$t_tinbai_mainsite->PK_TINBAI_ID->DbValue."','". ew_AdjustSql(trim($wrk))."','-20') ";	
			...
			$t_tinbai_mainsite->Row_Inserted($rsnew);
		}
