...
// Create page object
$t_lichcongtac_edit = new ct_lichcongtac_edit();
$Page =& $t_lichcongtac_edit;

// Page init
$t_lichcongtac_edit->Page_Init();

// Page main
$t_lichcongtac_edit->Page_Main();
?>
...
// Create page object
var t_lichcongtac_edit = new ew_Page("t_lichcongtac_edit");

// page properties
t_lichcongtac_edit.PageID = "edit"; // page ID
t_lichcongtac_edit.FormID = "ft_lichcongtacedit"; // form ID
var EW_PAGE_ID = t_lichcongtac_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
t_lichcongtac_edit.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_lichcongtac->C_CONTENT->FldCaption()) ?>");
...
//			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_lichcongtac->C_PARTICIPANTS_ID->FldCaption()) ?>");
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_lichcongtac->C_WHERE->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_C_PREPARED"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_lichcongtac->C_PREPARED->FldCaption()) ?>");
		elm = fobj.elements["x" + infix + "_C_OPTION"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($t_lichcongtac->C_OPTION->FldCaption()) ?>");
...
t_lichcongtac_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_lichcongtac_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_lichcongtac_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_lichcongtac_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="pheader"><?php echo $t_lichcongtac->TableCaption() ?><a style="text-align: right;float: right;font-size: 12px;" href="#" id="event_calendar">Danh sách sự kiện được đăng ký trong tuần</a></p>
<a href="<?php echo $t_lichcongtac->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a>
<?php include "event_calendar.php" ?>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_lichcongtac_edit->ShowMessage();
?>
<form name="ft_lichcongtacedit" id="ft_lichcongtacedit" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return t_lichcongtac_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="t_lichcongtac">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table cellspacing="0" class="ewTable">
<?php if ($t_lichcongtac->C_CADENLAR_ID->Visible) { // C_CADENLAR_ID ?>
    <tr style="display: none"<?php echo $t_lichcongtac->C_CADENLAR_ID->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_lichcongtac->C_CADENLAR_ID->FldCaption() ?></td>
		<td<?php echo $t_lichcongtac->C_CADENLAR_ID->CellAttributes() ?>><span id="el_C_CADENLAR_ID">
<div<?php echo $t_lichcongtac->C_CADENLAR_ID->ViewAttributes() ?>><?php echo $t_lichcongtac->C_CADENLAR_ID->EditValue ?></div><input type="hidden" name="x_C_CADENLAR_ID" id="x_C_CADENLAR_ID" value="<?php echo ew_HtmlEncode($t_lichcongtac->C_CADENLAR_ID->CurrentValue) ?>">
</span><?php echo $t_lichcongtac->C_CADENLAR_ID->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($t_lichcongtac->FK_CONGTY->Visible) { // FK_CONGTY ?>
	<tr<?php echo $t_lichcongtac->FK_CONGTY->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_lichcongtac->FK_CONGTY->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
		<td colspan="2" <?php echo $t_lichcongtac->FK_CONGTY->CellAttributes() ?>><span id="el_FK_CONGTY">
...
</span><?php echo $t_lichcongtac->FK_CONGTY->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($t_lichcongtac->FK_SB_ID->Visible) { // FK_SB_ID ?>
	<tr<?php echo $t_lichcongtac->FK_SB_ID->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_lichcongtac->FK_SB_ID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
                <td colspan="2" <?php echo $t_lichcongtac->FK_SB_ID->CellAttributes() ?>><span id="el_FK_SB_ID">
...
<?php if ($t_lichcongtac->C_TITLE->Visible) { // C_TITLE ?>
	<tr<?php echo $t_lichcongtac->C_TITLE->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_lichcongtac->C_TITLE->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
		<td colspan="2" <?php echo $t_lichcongtac->C_TITLE->CellAttributes() ?>><span id="el_C_TITLE">
<input type="text" name="x_C_TITLE" id="x_C_TITLE" title="<?php echo $t_lichcongtac->C_TITLE->FldTitle() ?>" size="98" maxlength="255" value="<?php echo $t_lichcongtac->C_TITLE->EditValue ?>"<?php echo $t_lichcongtac->C_TITLE->EditAttributes() ?>>
...
<?php if ($t_lichcongtac->C_DATE_STAR->Visible) { // C_DATE_STAR ?>
	<tr<?php echo $t_lichcongtac->C_DATE_STAR->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_lichcongtac->C_DATE_STAR->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
...
</span><?php echo $t_lichcongtac->C_DATE_STAR->CustomMsg ?>
      <span class="col2"> <?php echo $t_lichcongtac->C_HOUR_START->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?>&nbsp;</span>            
...
</span><?php echo $t_lichcongtac->C_HOUR_START->CustomMsg ?>              
...
</span><?php echo $t_lichcongtac->C_MINUTES_STAR->CustomMsg ?>                   
  <span class="col2"> <?php echo $t_lichcongtac->C_MINUTES_STAR->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span>    
...
</span><?php echo $t_lichcongtac->C_STATUS_STAR->CustomMsg ?>
...
<?php if ($t_lichcongtac->C_DATE_END->Visible) { // C_DATE_END ?>
	<tr<?php echo $t_lichcongtac->C_DATE_END->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_lichcongtac->C_DATE_END->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
...
</span><?php echo $t_lichcongtac->C_DATE_END->CustomMsg ?>
<span class="col2"> <?php echo $t_lichcongtac->C_HOUR_END->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span>  
...
</span><?php echo $t_lichcongtac->C_HOUR_END->CustomMsg ?>
...
</span><?php echo $t_lichcongtac->C_MINUTES_END->CustomMsg ?>  
<span class="col2"> <?php echo $t_lichcongtac->C_MINUTES_END->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span> 
                </td>
                <td>
...
</span><?php echo $t_lichcongtac->C_STATUS_END->CustomMsg ?>               
                </td>
	</tr>
<?php } ?>
...
<?php if ($t_lichcongtac->C_CONTENT->Visible) { // C_CONTENT ?>
	<tr<?php echo $t_lichcongtac->C_CONTENT->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_lichcongtac->C_CONTENT->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
		<td colspan="2" <?php echo $t_lichcongtac->C_CONTENT->CellAttributes() ?>><span id="el_C_CONTENT">
<textarea name="x_C_CONTENT" id="x_C_CONTENT" title="<?php echo $t_lichcongtac->C_CONTENT->FldTitle() ?>" cols="35" rows="4"<?php echo $t_lichcongtac->C_CONTENT->EditAttributes() ?>><?php echo $t_lichcongtac->C_CONTENT->EditValue ?></textarea>
<script type="text/javascript">
<!--
<?php if ($t_lichcongtac->C_CONTENT->ReadOnly) { ?>
...
</span><?php echo $t_lichcongtac->C_CONTENT->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($t_lichcongtac->C_ORGANIZATION->Visible) { // C_ORGANIZATION ?>
	<tr<?php echo $t_lichcongtac->C_ORGANIZATION->RowAttributes ?>>
		...
		<td colspan="2" <?php echo $t_lichcongtac->C_ORGANIZATION->CellAttributes() ?>><span id="el_C_ORGANIZATION">
 <textarea style="font-size: 12px" rows="2" cols="38" name="x_C_ORGANIZATION" id="x_C_ORGANIZATION" title="<?php echo $t_lichcongtac->C_ORGANIZATION->FldTitle() ?>" <?php echo $t_lichcongtac->C_ORGANIZATION->EditAttributes() ?>>
<?php echo $t_lichcongtac->C_ORGANIZATION->EditValue ?>
</textarea> 
</span><?php echo $t_lichcongtac->C_ORGANIZATION->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($t_lichcongtac->C_PARTICIPANTS_ID->Visible) { // C_PARTICIPANTS_ID ?>
	<tr<?php echo $t_lichcongtac->C_PARTICIPANTS_ID->RowAttributes ?>>
		...
		<td colspan="2" <?php echo $t_lichcongtac->C_PARTICIPANTS_ID->CellAttributes() ?>><span id="el_C_PARTICIPANTS_ID">
 <select tabindex="-1" class="x_C_PARTICIPANTS_ID" size="10" style="width:300px;" id="x_C_PARTICIPANTS_ID[]" name="x_C_PARTICIPANTS_ID[]" title="<?php echo $t_lichcongtac->C_PARTICIPANTS_ID->FldTitle() ?>" multiple="multiple"<?php echo $t_lichcongtac->C_PARTICIPANTS_ID->EditAttributes() ?>>
<?php
...
</span><?php echo $t_lichcongtac->C_PARTICIPANTS_ID->CustomMsg ?>
                    <i>(* Click hoặc gõ ký tự để chọn )</i>   </td>
	</tr>
<?php } ?>
<?php if ($t_lichcongtac->C_WHERE->Visible) { // C_WHERE ?>
	<tr<?php echo $t_lichcongtac->C_WHERE->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_lichcongtac->C_WHERE->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
		<td colspan="2" <?php echo $t_lichcongtac->C_WHERE->CellAttributes() ?>><span id="el_C_WHERE">
<input type="text" name="x_C_WHERE" id="x_C_WHERE" title="<?php echo $t_lichcongtac->C_WHERE->FldTitle() ?>" size="30" maxlength="255" value="<?php echo $t_lichcongtac->C_WHERE->EditValue ?>"<?php echo $t_lichcongtac->C_WHERE->EditAttributes() ?>>
</span><?php echo $t_lichcongtac->C_WHERE->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($t_lichcongtac->C_PREPARED->Visible) { // C_PREPARED ?>
	<tr<?php echo $t_lichcongtac->C_PREPARED->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_lichcongtac->C_PREPARED->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
		<td colspan="2" <?php echo $t_lichcongtac->C_PREPARED->CellAttributes() ?>><span id="el_C_PREPARED">
<input type="text" name="x_C_PREPARED" id="x_C_PREPARED" title="<?php echo $t_lichcongtac->C_PREPARED->FldTitle() ?>" size="30" maxlength="255" value="<?php echo $t_lichcongtac->C_PREPARED->EditValue ?>"<?php echo $t_lichcongtac->C_PREPARED->EditAttributes() ?>>
</span><?php echo $t_lichcongtac->C_PREPARED->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($t_lichcongtac->C_OPTION->Visible) { // C_OPTION ?>
	<tr<?php echo $t_lichcongtac->C_OPTION->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_lichcongtac->C_OPTION->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
		<td colspan="2"<?php echo $t_lichcongtac->C_OPTION->CellAttributes() ?>><span id="el_C_OPTION">
<div id="tp_x_C_OPTION" class="<?php echo EW_ITEM_TEMPLATE_CLASSNAME ?>"><label><input type="radio" name="x_C_OPTION" id="x_C_OPTION" title="<?php echo $t_lichcongtac->C_OPTION->FldTitle() ?>" value="{value}"<?php echo $t_lichcongtac->C_OPTION->EditAttributes() ?>></label></div>
<div id="dsl_x_C_OPTION" repeatcolumn="5">
<?php
$arwrk = $t_lichcongtac->C_OPTION->EditValue;
if (is_array($arwrk)) {
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($t_lichcongtac->C_OPTION->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;

		// Note: No spacing within the LABEL tag
?>
<?php echo ew_RepeatColumnTable($rowswrk, $rowcntwrk, 5, 1) ?>
<label><input type="radio" name="x_C_OPTION" id="x_C_OPTION" title="<?php echo $t_lichcongtac->C_OPTION->FldTitle() ?>" value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?><?php echo $t_lichcongtac->C_OPTION->EditAttributes() ?>><?php echo $arwrk[$rowcntwrk][1] ?></label>
<?php echo ew_RepeatColumnTable($rowswrk, $rowcntwrk, 5, 2) ?>
<?php
	}
}
...
</span><?php echo $t_lichcongtac->C_OPTION->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($t_lichcongtac->C_FILE_ATTACH->Visible) { // C_FILE_ATTACH ?>
	<tr<?php echo $t_lichcongtac->C_FILE_ATTACH->RowAttributes ?>>
		...
		<td colspan="2"<?php echo $t_lichcongtac->C_FILE_ATTACH->CellAttributes() ?>><span id="el_C_FILE_ATTACH">
...
<a href="<?php echo $t_lichcongtac->C_FILE_ATTACH->HrefValue ?>"><?php echo $t_lichcongtac->C_FILE_ATTACH->EditValue ?></a>
<?php } elseif (!in_array($t_lichcongtac->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($t_lichcongtac->C_FILE_ATTACH->Upload->DbValue)) { ?>
<?php echo $t_lichcongtac->C_FILE_ATTACH->EditValue ?>
<?php } elseif (!in_array($t_lichcongtac->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<label><input type="radio" name="a_C_FILE_ATTACH" id="a_C_FILE_ATTACH" value="2"><?php echo $Language->Phrase("Remove") ?></label>&nbsp;
<label><input type="radio" name="a_C_FILE_ATTACH" id="a_C_FILE_ATTACH" value="3"><?php echo $Language->Phrase("Replace") ?><br></label>
<?php $t_lichcongtac->C_FILE_ATTACH->EditAttrs["onchange"] = "this.form.a_C_FILE_ATTACH[2].checked=true;" . @$t_lichcongtac->C_FILE_ATTACH->EditAttrs["onchange"]; ?>
<?php } else { ?>
<input type="hidden" name="a_C_FILE_ATTACH" id="a_C_FILE_ATTACH" value="3">
<?php } ?>
<input type="file" name="x_C_FILE_ATTACH" id="x_C_FILE_ATTACH" title="<?php echo $t_lichcongtac->C_FILE_ATTACH->FldTitle() ?>" size="30"<?php echo $t_lichcongtac->C_FILE_ATTACH->EditAttributes() ?>>
</div>
</span><?php echo $t_lichcongtac->C_FILE_ATTACH->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($t_lichcongtac->C_ACTIVE->Visible) { // C_ACTIVE ?>
	<tr<?php echo $t_lichcongtac->C_ACTIVE->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_lichcongtac->C_ACTIVE->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
		<td colspan="2" <?php echo $t_lichcongtac->C_ACTIVE->CellAttributes() ?>><span id="el_C_ACTIVE">
...
<?php if ($t_lichcongtac->C_PUBLISH->Visible) { // C_PUBLISH ?>
	<tr<?php echo $t_lichcongtac->C_PUBLISH->RowAttributes ?>>
		<td class="ewTableHeader"><?php echo $t_lichcongtac->C_PUBLISH->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td>
		<td colspan="2" <?php echo $t_lichcongtac->C_PUBLISH->CellAttributes() ?>><span id="el_C_PUBLISH">
...
<?php
$t_lichcongtac_edit->Page_Terminate();
?>
<?php
...
class ct_lichcongtac_edit {
	...
	// Page object name
	var $PageObjName = 't_lichcongtac_edit';
	...
	function Page_Init() {
		...
		global $t_lichcongtac, $C_CADENLAR_ID;
                $C_CADENLAR_ID = ew_HtmlEncode(@$_GET["C_CADENLAR_ID"]);
		...
		if (!$Security->CanEdit()) {
			...
			$this->Page_Terminate("t_lichcongtaclist.php");
                ...
                //echo  $Security->CanEdit();
              
                $a= $Security->CanEdit_Report($Security->CurrentUserLevelID(),$C_CADENLAR_ID);
                if ($a==0) {
			$Security->SaveLastUrl();
			$this->Page_Terminate("t_lichcongtaclist.php");
		}
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$t_lichcongtac->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->GetUploadFiles(); // Get upload files
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$t_lichcongtac->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$t_lichcongtac->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$t_lichcongtac->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($t_lichcongtac->C_CADENLAR_ID->CurrentValue == "")
			$this->Page_Terminate("t_lichcongtaclist.php"); // Invalid key, return to list
		switch ($t_lichcongtac->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					$this->setMessage($Language->Phrase("NoRecord")); // No record found
					$this->Page_Terminate("t_lichcongtaclist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$t_lichcongtac->SendEmail = TRUE; // Send email on update success
				if ($this->EditRow()) { // Update record based on key
					$this->setMessage($Language->Phrase("UpdateSuccess")); // Update success
					$sReturnUrl = $t_lichcongtac->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "t_lichcongtacview.php")
						$sReturnUrl = $t_lichcongtac->ViewUrl(); // View paging, return to View page directly
		...
		// Render the record
		$t_lichcongtac->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function GetUploadFiles() {
		...
		// Get upload data
			if ($t_lichcongtac->C_FILE_ATTACH->Upload->UploadFile()) {

				// No action required
			} else {
				echo $t_lichcongtac->C_FILE_ATTACH->Upload->Message;
				$this->Page_Terminate();
				exit();
			}
	}
	...
	function LoadFormValues() {
		...
		$t_lichcongtac->FK_CONGTY->setFormValue($objForm->GetValue("x_FK_CONGTY"));
		...
		$t_lichcongtac->C_CONTENT->setFormValue($objForm->GetValue("x_C_CONTENT"));
		$t_lichcongtac->C_ORGANIZATION->setFormValue($objForm->GetValue("x_C_ORGANIZATION"));
		$t_lichcongtac->C_PARTICIPANTS_ID->setFormValue($objForm->GetValue("x_C_PARTICIPANTS_ID"));
		$t_lichcongtac->C_WHERE->setFormValue($objForm->GetValue("x_C_WHERE"));
		$t_lichcongtac->C_PREPARED->setFormValue($objForm->GetValue("x_C_PREPARED"));
		$t_lichcongtac->C_OPTION->setFormValue($objForm->GetValue("x_C_OPTION"));
		...
		$t_lichcongtac->C_USER_EDIT->setFormValue($objForm->GetValue("x_C_USER_EDIT"));
		$t_lichcongtac->C_EDIT_TIME->setFormValue($objForm->GetValue("x_C_EDIT_TIME"));
		$t_lichcongtac->C_EDIT_TIME->CurrentValue = ew_UnFormatDateTime($t_lichcongtac->C_EDIT_TIME->CurrentValue, 7);
	}
	...
	function RestoreFormValues() {
		...
		$t_lichcongtac->FK_CONGTY->CurrentValue = $t_lichcongtac->FK_CONGTY->FormValue;
		...
		$t_lichcongtac->C_CONTENT->CurrentValue = $t_lichcongtac->C_CONTENT->FormValue;
		$t_lichcongtac->C_ORGANIZATION->CurrentValue = $t_lichcongtac->C_ORGANIZATION->FormValue;
		$t_lichcongtac->C_PARTICIPANTS_ID->CurrentValue = $t_lichcongtac->C_PARTICIPANTS_ID->FormValue;
		$t_lichcongtac->C_WHERE->CurrentValue = $t_lichcongtac->C_WHERE->FormValue;
		$t_lichcongtac->C_PREPARED->CurrentValue = $t_lichcongtac->C_PREPARED->FormValue;
		$t_lichcongtac->C_OPTION->CurrentValue = $t_lichcongtac->C_OPTION->FormValue;
		...
		$t_lichcongtac->C_USER_EDIT->CurrentValue = $t_lichcongtac->C_USER_EDIT->FormValue;
		$t_lichcongtac->C_EDIT_TIME->CurrentValue = $t_lichcongtac->C_EDIT_TIME->FormValue;
		$t_lichcongtac->C_EDIT_TIME->CurrentValue = ew_UnFormatDateTime($t_lichcongtac->C_EDIT_TIME->CurrentValue, 7);
	}
	...
	function RenderRow() {
		...
		if ($t_lichcongtac->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// C_CADENLAR_ID
			$t_lichcongtac->C_CADENLAR_ID->EditCustomAttributes = "";
			$t_lichcongtac->C_CADENLAR_ID->EditValue = $t_lichcongtac->C_CADENLAR_ID->CurrentValue;
			$t_lichcongtac->C_CADENLAR_ID->CssStyle = "";
			$t_lichcongtac->C_CADENLAR_ID->CssClass = "";
			$t_lichcongtac->C_CADENLAR_ID->ViewCustomAttributes = "";
			...
			// C_CONTENT
			$t_lichcongtac->C_CONTENT->EditCustomAttributes = "";
			$t_lichcongtac->C_CONTENT->EditValue = ew_HtmlEncode($t_lichcongtac->C_CONTENT->CurrentValue);

			// C_ORGANIZATION
			$t_lichcongtac->C_ORGANIZATION->EditCustomAttributes = "";
			$t_lichcongtac->C_ORGANIZATION->EditValue = ew_HtmlEncode($t_lichcongtac->C_ORGANIZATION->CurrentValue);
			...
			$t_lichcongtac->C_WHERE->EditValue = ew_HtmlEncode($t_lichcongtac->C_WHERE->CurrentValue);

			// C_PREPARED
			$t_lichcongtac->C_PREPARED->EditCustomAttributes = "";
			$t_lichcongtac->C_PREPARED->EditValue = ew_HtmlEncode($t_lichcongtac->C_PREPARED->CurrentValue);
			...
			// C_FILE_ATTACH
			$t_lichcongtac->C_FILE_ATTACH->EditCustomAttributes = "";
			if (!ew_Empty($t_lichcongtac->C_FILE_ATTACH->Upload->DbValue)) {
				$t_lichcongtac->C_FILE_ATTACH->EditValue = $t_lichcongtac->C_FILE_ATTACH->Upload->DbValue;
			} else {
				$t_lichcongtac->C_FILE_ATTACH->EditValue = "";
			}
			...
			$arwrk[] = array("1", "Duyệt lên lịch trường");
			...
			// C_FILE_ATTACH
			if (!ew_Empty($t_lichcongtac->C_FILE_ATTACH->Upload->DbValue)) {
				$t_lichcongtac->C_FILE_ATTACH->HrefValue = ew_UploadPathEx(FALSE, $t_lichcongtac->C_FILE_ATTACH->UploadPath) . ((!empty($t_lichcongtac->C_FILE_ATTACH->EditValue)) ? $t_lichcongtac->C_FILE_ATTACH->EditValue : $t_lichcongtac->C_FILE_ATTACH->CurrentValue);
				if ($t_lichcongtac->Export <> "") $t_lichcongtac->C_FILE_ATTACH->HrefValue = ew_ConvertFullUrl($t_lichcongtac->C_FILE_ATTACH->HrefValue);
			} else {
				$t_lichcongtac->C_FILE_ATTACH->HrefValue = "";
			}
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		if (!ew_CheckFileType($t_lichcongtac->C_FILE_ATTACH->Upload->FileName)) {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("WrongFileType");
		}
		if ($t_lichcongtac->C_FILE_ATTACH->Upload->FileSize > 0 && EW_MAX_FILE_SIZE > 0 && $t_lichcongtac->C_FILE_ATTACH->Upload->FileSize > EW_MAX_FILE_SIZE) {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= str_replace("%s", EW_MAX_FILE_SIZE, $Language->Phrase("MaxFileSize"));
		}
		if (in_array($t_lichcongtac->C_FILE_ATTACH->Upload->Error, array(1, 2, 3, 6, 7, 8))) {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("PhpUploadErr" . $t_lichcongtac->C_FILE_ATTACH->Upload->Error);
		}

		// Check if validation required
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!is_null($t_lichcongtac->FK_SB_ID->FormValue) && $t_lichcongtac->FK_SB_ID->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac->FK_SB_ID->FldCaption();
		}
		if (!is_null($t_lichcongtac->C_TITLE->FormValue) && $t_lichcongtac->C_TITLE->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac->C_TITLE->FldCaption();
		}
		if (!is_null($t_lichcongtac->C_DATE_STAR->FormValue) && $t_lichcongtac->C_DATE_STAR->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
		...
		if (!is_null($t_lichcongtac->C_DATE_END->FormValue) && $t_lichcongtac->C_DATE_END->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
		...
		if (!is_null($t_lichcongtac->C_CONTENT->FormValue) && $t_lichcongtac->C_CONTENT->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac->C_CONTENT->FldCaption();
		}

//		if ($t_lichcongtac->C_PARTICIPANTS_ID->FormValue == "") {
//			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
//			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac->C_PARTICIPANTS_ID->FldCaption();
//		}
		if (!is_null($t_lichcongtac->C_WHERE->FormValue) && $t_lichcongtac->C_WHERE->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac->C_WHERE->FldCaption();
		}
		if (!is_null($t_lichcongtac->C_PREPARED->FormValue) && $t_lichcongtac->C_PREPARED->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac->C_PREPARED->FldCaption();
		}
		if ($t_lichcongtac->C_OPTION->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= $Language->Phrase("EnterRequiredField") . " - " . $t_lichcongtac->C_OPTION->FldCaption();
		}
		if ($t_lichcongtac->C_ACTIVE->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// FK_CONGTY
			$t_lichcongtac->FK_CONGTY->SetDbValueDef($rsnew, $t_lichcongtac->FK_CONGTY->CurrentValue, NULL, FALSE);
			...
			$t_lichcongtac->C_DATE_STAR->SetDbValueDef($rsnew, ew_UnFormatDateTime($t_lichcongtac->C_DATE_STAR->CurrentValue, 7, FALSE), NULL);
			 ...
			 if (!isset($t_lichcongtac->C_STATUS_STAR->CurrentValue)|| ($t_lichcongtac->C_STATUS_STAR->CurrentValue == 1))
                            {
                            // C_HOUR_START
                            $t_lichcongtac->C_HOUR_START->SetDbValueDef($rsnew, $t_lichcongtac->C_HOUR_START->CurrentValue, NULL, FALSE);
                            // C_MINUTES_STAR
                            if ($t_lichcongtac->C_MINUTES_STAR->CurrentValue == 0) $t_lichcongtac->C_MINUTES_STAR->CurrentValue ="0".$t_lichcongtac->C_MINUTES_STAR->CurrentValue;
                            $t_lichcongtac->C_MINUTES_STAR->SetDbValueDef($rsnew, $t_lichcongtac->C_MINUTES_STAR->CurrentValue, NULL, FALSE);
                            } 
                            else 
                            {
                            // C_HOUR_START
                            $t_lichcongtac->C_HOUR_START->SetDbValueDef($rsnew, NULL, NULL, FALSE);
                            // C_MINUTES_STAR
                            $t_lichcongtac->C_MINUTES_STAR->SetDbValueDef($rsnew, NULL, NULL, FALSE);    
                            } 
			...
			// C_STATUS_END
                            ...
                            if (!isset($t_lichcongtac->C_STATUS_END->CurrentValue) || ($t_lichcongtac->C_STATUS_END->CurrentValue == 1))
                            {
                            // C_HOUR_END
                            $t_lichcongtac->C_HOUR_END->SetDbValueDef($rsnew, $t_lichcongtac->C_HOUR_END->CurrentValue, NULL, FALSE);
                            // C_MINUTES_END
                            if ($t_lichcongtac->C_MINUTES_END->CurrentValue == 0) $t_lichcongtac->C_MINUTES_END->CurrentValue ="0".$t_lichcongtac->C_MINUTES_END->CurrentValue;
                            $t_lichcongtac->C_MINUTES_END->SetDbValueDef($rsnew, $t_lichcongtac->C_MINUTES_END->CurrentValue, NULL, FALSE);
                            } 
                            else 
                            {
                            // C_HOUR_END
                            $t_lichcongtac->C_HOUR_END->SetDbValueDef($rsnew, NULL, NULL, FALSE);
                            // C_MINUTES_END
                            $t_lichcongtac->C_MINUTES_END->SetDbValueDef($rsnew, NULL, NULL, FALSE);
                            }
			// C_CONTENT
			$t_lichcongtac->C_CONTENT->SetDbValueDef($rsnew, $t_lichcongtac->C_CONTENT->CurrentValue, NULL, FALSE);

			// C_ORGANIZATION
			$t_lichcongtac->C_ORGANIZATION->SetDbValueDef($rsnew, $t_lichcongtac->C_ORGANIZATION->CurrentValue, NULL, FALSE);

			// C_PARTICIPANTS_ID
			$t_lichcongtac->C_PARTICIPANTS_ID->SetDbValueDef($rsnew, $t_lichcongtac->C_PARTICIPANTS_ID->CurrentValue, NULL, FALSE);

			// C_WHERE
			$t_lichcongtac->C_WHERE->SetDbValueDef($rsnew, $t_lichcongtac->C_WHERE->CurrentValue, NULL, FALSE);

			// C_PREPARED
			$t_lichcongtac->C_PREPARED->SetDbValueDef($rsnew, $t_lichcongtac->C_PREPARED->CurrentValue, NULL, FALSE);

			// C_OPTION
			$t_lichcongtac->C_OPTION->SetDbValueDef($rsnew, $t_lichcongtac->C_OPTION->CurrentValue, NULL, FALSE);

			// C_FILE_ATTACH
			$t_lichcongtac->C_FILE_ATTACH->Upload->SaveToSession(); // Save file value to Session
						if ($t_lichcongtac->C_FILE_ATTACH->Upload->Action == "2" || $t_lichcongtac->C_FILE_ATTACH->Upload->Action == "3") { // Update/Remove
			$t_lichcongtac->C_FILE_ATTACH->Upload->DbValue = $rs->fields('C_FILE_ATTACH'); // Get original value
			if (is_null($t_lichcongtac->C_FILE_ATTACH->Upload->Value)) {
				$rsnew['C_FILE_ATTACH'] = NULL;
			} else {
				$rsnew['C_FILE_ATTACH'] = ew_UploadFileNameEx(ew_UploadPathEx(TRUE, $t_lichcongtac->C_FILE_ATTACH->UploadPath), $t_lichcongtac->C_FILE_ATTACH->Upload->FileName);
			}
			}
			...
			// C_FK_PUBLISH
                            if ($t_lichcongtac->C_PUBLISH->CurrentValue == '1')
                            {
                                $t_lichcongtac->C_FK_PUBLISH->SetDbValueDef($rsnew, 1, NULL);
                                // C_DATETIME_PUBLISH
                            ...
                            {
                                $t_lichcongtac->C_FK_PUBLISH->SetDbValueDef($rsnew, 0, NULL);
                                // C_DATETIME_PUBLISH
                                $t_lichcongtac->C_DATETIME_PUBLISH->SetDbValueDef($rsnew, NULL, NULL);
                                $rsnew['C_DATETIME_PUBLISH'] =& $t_lichcongtac->C_DATETIME_PUBLISH->DbValue;  
			...
			// C_USER_EDIT
			$t_lichcongtac->C_USER_EDIT->SetDbValueDef($rsnew, CurrentUserID(), NULL);
			$rsnew['C_USER_EDIT'] =& $t_lichcongtac->C_USER_EDIT->DbValue;

			// C_EDIT_TIME
			$t_lichcongtac->C_EDIT_TIME->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
			$rsnew['C_EDIT_TIME'] =& $t_lichcongtac->C_EDIT_TIME->DbValue;

			// Call Row Updating event
			$bUpdateRow = $t_lichcongtac->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
			if (!ew_Empty($t_lichcongtac->C_FILE_ATTACH->Upload->Value)) {
				$t_lichcongtac->C_FILE_ATTACH->Upload->SaveToFile($t_lichcongtac->C_FILE_ATTACH->UploadPath, $rsnew['C_FILE_ATTACH'], FALSE);
			}
				$conn->raiseErrorFn = 'ew_ErrorFn';
//                                echo $t_lichcongtac->UpdateSQL($rsnew);
//                                exit();
		...
		// C_FILE_ATTACH
		$t_lichcongtac->C_FILE_ATTACH->Upload->RemoveFromSession(); // Remove file value from Session
		return $EditRow;
	}
