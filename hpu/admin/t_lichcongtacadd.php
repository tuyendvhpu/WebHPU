...
// Create page object
$t_lichcongtac_add = new ct_lichcongtac_add();
$Page =& $t_lichcongtac_add;

// Page init
$t_lichcongtac_add->Page_Init();

// Page main
$t_lichcongtac_add->Page_Main();
?>
...
// Create page object
var t_lichcongtac_add = new ew_Page("t_lichcongtac_add");

// page properties
t_lichcongtac_add.PageID = "add"; // page ID
t_lichcongtac_add.FormID = "ft_lichcongtacadd"; // form ID
var EW_PAGE_ID = t_lichcongtac_add.PageID; // for backward compatibility

// extend page with ValidateForm function
t_lichcongtac_add.ValidateForm = function(fobj) {
...
//		elm = fobj.elements["x" + infix + "_C_FILE_ATTACH"];
...
t_lichcongtac_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_lichcongtac_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_lichcongtac_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_lichcongtac_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<script src="js/select2.js"></script>
    ...
    <script id="script_e14">
        $(document).ready(function () {
            $(".x_C_PARTICIPANTS_ID").val(["AL","AZ"]).select2();
            $("#e8_get").click(function() { $("#e14").select2(); });
            $("#e8_get1").click(function() { alert("Selected value is: "+$("#e14").select2("val"));});
        });
         $("#e8_get1").click(function () { alert("Selected value is: "+$("#e14").select2("val"));});
...
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$t_lichcongtac_add->ShowMessage();
?>
<form name="ft_lichcongtacadd" id="ft_lichcongtacadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return t_lichcongtac_add.ValidateForm(this);">
<p>
...
<?php if ($t_lichcongtac->C_PARTICIPANTS_ID->Visible) { // C_PARTICIPANTS_ID ?>
 ...
 <select tabindex="-1" class="x_C_PARTICIPANTS_ID" multiple="multiple" style="width:300px" id="x_C_PARTICIPANTS_ID[]" name="x_C_PARTICIPANTS_ID[]" class="populate select2-offscreen">
<?php
...
?>
        ...
        <i>(* Click hoặc gõ ký tự để chọn )</i>       
                </td>
	</tr>
<?php } ?>
...
<input type="text" name="x_C_WHERE" id="x_C_WHERE" title="<?php echo $t_lichcongtac->C_WHERE->FldTitle() ?>" size="98" maxlength="255" value="<?php echo $t_lichcongtac->C_WHERE->EditValue ?>"<?php echo $t_lichcongtac->C_WHERE->EditAttributes() ?>>
...
<input type="text" name="x_C_PREPARED" id="x_C_PREPARED" title="<?php echo $t_lichcongtac->C_PREPARED->FldTitle() ?>" size="98" maxlength="255" value="<?php echo $t_lichcongtac->C_PREPARED->EditValue ?>"<?php echo $t_lichcongtac->C_PREPARED->EditAttributes() ?>>
...
<?php
$t_lichcongtac_add->Page_Terminate();
?>
<?php
...
class ct_lichcongtac_add {
	...
	// Page object name
	var $PageObjName = 't_lichcongtac_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $t_lichcongtac->CurrentAction = $_POST["a_add"]; // Get form action
		  ...
		  if ($bCopy) {
		    $t_lichcongtac->CurrentAction = "C"; // Copy record
		  } else {
		    $t_lichcongtac->CurrentAction = "I"; // Display blank record
		...
		switch ($t_lichcongtac->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$t_lichcongtac->SendEmail = TRUE; // Send email on add success
		    if ($this->AddRow()) { // Add successful
		      $this->setMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $t_lichcongtac->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "t_lichcongtacview.php")
						$sReturnUrl = $t_lichcongtac->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$t_lichcongtac->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $t_lichcongtac;
		$t_lichcongtac->FK_SB_ID->CurrentValue = 3;
		$t_lichcongtac->C_FILE_ATTACH->CurrentValue = NULL; // Clear file related field
		$t_lichcongtac->C_ACTIVE->CurrentValue = 1;
                $t_lichcongtac->C_PUBLISH->CurrentValue = 0;
                $t_lichcongtac->C_OPTION->CurrentValue = 0;
	}
	...
	function LoadFormValues() {
		...
		$t_lichcongtac->C_USER_ADD->setFormValue($objForm->GetValue("x_C_USER_ADD"));
		$t_lichcongtac->C_ADD_TIME->setFormValue($objForm->GetValue("x_C_ADD_TIME"));
		$t_lichcongtac->C_ADD_TIME->CurrentValue = ew_UnFormatDateTime($t_lichcongtac->C_ADD_TIME->CurrentValue, 7);
	...
	function RestoreFormValues() {
		...
		$t_lichcongtac->C_USER_ADD->CurrentValue = $t_lichcongtac->C_USER_ADD->FormValue;
		$t_lichcongtac->C_ADD_TIME->CurrentValue = $t_lichcongtac->C_ADD_TIME->FormValue;
		$t_lichcongtac->C_ADD_TIME->CurrentValue = ew_UnFormatDateTime($t_lichcongtac->C_ADD_TIME->CurrentValue, 7);
	}
	...
	function RenderRow() {
		...
		} elseif ($t_lichcongtac->RowType == EW_ROWTYPE_ADD) { // Add row
...
//		}
	...
	function AddRow() {
		...
		// C_USER_ADD
		$t_lichcongtac->C_USER_ADD->SetDbValueDef($rsnew, CurrentUserID(), NULL);
		$rsnew['C_USER_ADD'] =& $t_lichcongtac->C_USER_ADD->DbValue;

		// C_ADD_TIME
		$t_lichcongtac->C_ADD_TIME->SetDbValueDef($rsnew, ew_CurrentDateTime(), NULL);
		$rsnew['C_ADD_TIME'] =& $t_lichcongtac->C_ADD_TIME->DbValue;

		// Call Row Inserting event
		$bInsertRow = $t_lichcongtac->Row_Inserting($rsnew);
		if ($bInsertRow) {
			...
			$AddRow = $conn->Execute($t_lichcongtac->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$t_lichcongtac->C_CADENLAR_ID->setDbValue($conn->Insert_ID());
			$rsnew['C_CADENLAR_ID'] = $t_lichcongtac->C_CADENLAR_ID->DbValue;

			// Call Row Inserted event
			$t_lichcongtac->Row_Inserted($rsnew);
		}
