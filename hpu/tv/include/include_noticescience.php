...
?>    
 ...
 <?php
                $sSqlWrk = "SELECT t_thongbao_levelsite.PK_THONGBAO,
                        ...
                        FROM t_thongbao_levelsite INNER JOIN t_nguoidung ON (t_thongbao_levelsite.C_USER_ADD = t_nguoidung.PK_NGUOIDUNG_ID)
                        Inner Join t_congty On t_thongbao_levelsite.FK_CONGTY_ID = t_congty.PK_MACONGTY";
                        $sWhereWrk = "(t_thongbao_levelsite.PK_THONGBAO NOT IN ".$string_pk_thongbao.") AND (t_thongbao_levelsite.PK_CHUYENMUC_ID='".KillChars(ew_HtmlEncode($_GET['subid']))."') AND (t_thongbao_levelsite.FK_CONGTY_ID IN ".$_SESSION['FK_CONGTY_EX'].") AND ( t_thongbao_levelsite.C_ACTIVE=1) LIMIT 7";
                            if ($sWhereWrk <> "") $sSqlWrk .= " WHERE $sWhereWrk";
                            $rswrk = $conn->Execute($sSqlWrk);
                            $arwrk = ($rswrk) ? $rswrk->GetRows() : array();
                            if ($rswrk) $rswrk->Close();
...
?>                  <h2 class="h2title"> Thông báo liên quan</h2>       
                    <ul class="ulcontents1">
                           <?php for($k=0;$k<$rowswrk;$k++) { 
                                   ...
                                   $url ="TT-Thongtin-Thuvienthongbao-".$arwrk[$k]['PK_THONGBAO']."-".$obid."-".$subid."-".$fk_congty."-".changeTitle($arwrk[$k]['C_TITLE']).".html";
                                  ?> 
