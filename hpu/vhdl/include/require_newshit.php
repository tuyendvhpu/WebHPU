...
<div class="adv">
                    ...
                    <ul class="ulcontents1">
                             <?php for($k=0;$k<$rowswrk;$k++) { 
                                  ...
                                  $url ="VHDLtintuc-".$arwrk[$k]['PK_TINBAI_ID']."-".$obid."-".$subid."-".$atid."-".changeTitle($arwrk[$k]['C_TITLE']).".html";           
                                  ?> 
 ...
 <?php
   ...
   $string_string_pk = "(".substr($string_string_pk,1).")"; 
	$sSqlWrk = "Select t_tinbai_mainlevel.*, t_tinbai_levelsite.C_ORDER,t_tinbai_levelsite.C_ACTIVE,
                            ...
                            From t_tinbai_mainlevel Inner Join
                            t_tinbai_levelsite On t_tinbai_mainlevel.PK_TINBAI_ID = t_tinbai_levelsite.PK_TINBAI_ID  ";
                            $sWhereWrk = "(t_tinbai_mainlevel.PK_TINBAI_ID NOT IN ".$string_string_pk." ) AND (t_tinbai_mainlevel.FK_CONGTY = ".$_SESSION['FK_CONGTY'].") AND (t_tinbai_mainlevel.FK_NGUOIDUNG_ID IS NOT NULL) AND (t_tinbai_levelsite.C_ACTIVE =1) ORDER BY t_tinbai_mainlevel.C_ORDER DESC LIMIT 5";
                            if ($sWhereWrk <> "") $sSqlWrk .= " WHERE $sWhereWrk";
                            $rswrk = $conn->Execute($sSqlWrk);
                            $arwrk = ($rswrk) ? $rswrk->GetRows() : array();
                            if ($rswrk) $rswrk->Close();
