 ...
 <div id="layout" class="clearfix">
       ...
       <div id="contentbody" class="clearfix">
            <div id="titlepages" style="margin:0px 7px 0px 7px;background:white">
                <h2 class="h2title-detail"> <a href="#">ĐẠI HỌC DÂN LẬP HẢI PHÒNG </a> - <a href="list_notice.php"> LỊCH CÔNG TÁC </a></h2>
            </div>
...
<?php
 ...
 $Date = KillChars(ew_HtmlEncode($_GET['date'])); 
 ...
 $EndDateByNumberOfWeek = date ('Y-m-j' , $new_date );
    ...
    $calendar_user="SELECT * FROM `t_lichcongtac` WHERE (t_lichcongtac.FK_CONGTY = '".$_SESSION['FK_CONGTY']."') AND (t_lichcongtac.C_ACTIVE = '1')  AND ((`C_DATE_STAR` BETWEEN '".$beginday."' AND '".$EndDateByNumberOfWeek."')) ORDER BY C_DATE_STAR,C_HOUR_START"; 
    $rswrk1 = $conn->Execute($calendar_user);
    $arwrk1 = ($rswrk1) ? $rswrk1->GetRows() : array();
      if ($rswrk1) $rswrk1->Close();
    $calendar_parent="SELECT * FROM `t_lichcongtac` WHERE ((t_lichcongtac.C_PARTICIPANTS_ID LIKE '%".$_SESSION['FK_CONGTY']."x%')) AND (t_lichcongtac.FK_CONGTY <> '115') AND (t_lichcongtac.C_PUBLISH = '1') AND ((`C_DATE_STAR` BETWEEN '".$beginday."' AND '".$EndDateByNumberOfWeek."')) ORDER BY C_DATE_STAR,C_HOUR_START"; 
			...
			$t_ghichu_lich->C_WEEK->EditValue = $arwrk;
               ...
               ?>
                ...
                <tr style="background: url('../images/khoa/top-nav.jpg') repeat-x; height:28px;">
                               ...
                               <td style="width:290px;text-align: center"> <b>Nội dung</b></td>
                               <td style="width:70px;text-align: center"> <b>Chuẩn bị</b></td>
	 ...
	 <?php for($i=0;$i<count($array_day);$i++) 
               ...
               // xac dinh ngay hien tai
                              ...
                              else 
                            {
                              $style_active="";
                              $numDaysToMon = $numDaysToMon;
                               $tr_background = "style=\"background:#E0FFFF;display:none \"";
                     ...
                     switch ($array[$k]['C_STATUS_END'])
                            ...
                            default:
                             $t_lichcongtac->C_THOIGIAN->ViewValue1 = "Từ: ".$array[$k]['C_HOUR_START']."h".$array[$k]['C_MINUTES_STAR']."-".$array[$k]['C_HOUR_END']."h".$array[$k]['C_MINUTES_END'];  
                                ...
                                <tr>
                                    <td style="width:290px"><a class="calender_title" href="calendarevent.php?eventid=<?php echo $array[$k]['C_CADENLAR_ID']  ?>"> <SPAN class="spanc_thoigian"><?php echo $t_lichcongtac->C_THOIGIAN->ViewValue1 ?></SPAN>&nbsp;<?php echo $array[$k]['C_TITLE'] ?> </a></td>
                                    <td style="width:70px"><span class="spantext"> <?php echo $t_lichcongtac->FK_CONGTY->ViewValue1  ?></span></td>
                                    <td style="width:170px"><span class="spantext"><?php echo $t_lichcongtac->C_PARTICIPANTS_ID->ViewValue1 ;?></span></td>
			...
			<td colspan="4">
                                 ...
                                 <?php
                                        ...
                                        // C_PARTICIPANTS_ID             
                                              ...
                                              <?php if (($array[$k]['C_HOUR_START'] > 12) && ($array[$k]['C_HOUR_START'] <=24))
                                                                    {  ?>
                                <tr>
                                    <td style="width:290px"><a class="calender_title" href="calendarevent.php?eventid=<?php echo $array[$k]['C_CADENLAR_ID']  ?>"><?php echo  $t_lichcongtac->C_THOIGIAN->ViewValue1." ".$array[$k]['C_TITLE'] ?> </a></td>
                                    <td style="width:70px"><span class="spantext"> <?php echo  $t_lichcongtac->FK_CONGTY->ViewValue1   ?></span></td>
                                    <td style="width:170px"><span class="spantext"><?php echo $t_lichcongtac->C_PARTICIPANTS_ID->ViewValue1 ?></span></td>
                                     <td style="width:120px"><span class="spantext"><?php echo $array[$k]['C_WHERE'] ?></span></td>
                                </tr>
               ...
               <td style="width:250px;vertical-align: top">
                      <div style="border:1px #c2d4da solid;font-family:Tahoma;">
                       <img src="../images/khoa/img_lichphongban.jpg">
                       <img src="../images/khoa/img_nhomlich.jpg">
                       <img src="../images/khoa/img_timkiemnhanh1.jpg">
                        ...
                        $(function() {
                                $('#inlineDatepicker').datepick({multiSelect: 4,showTrigger: '#calImg',dateFormat: 'yyyy-mm-dd',onSelect: showDate});
                        });
                        ...
                        window.location = "Doanthelich-ngay.html?date="+newdate+"";
