                ...
                if (($subid <> 0))
                          {
                            $title= stringname($subid,'C_NAME','t_chuyenmuc_levelsite','PK_CHUYENMUC');
                              $url ="Doanthechuyenmuc-".$subid."-". $_SESSION['FK_CONGTY']."-".changeTitle($title).".html";  
                          }
                          else
                          {
                              $title= stringname($obid,'C_NAME','t_doituong_levelsite','PK_DOITUONG'); 
                               $url ="Doanthethongbao.html";  
                          }    
                ?>
                <h2 class="h2title-detail"> <a href="<?php echo $url ?>" style="text-transform: uppercase"> <?php echo $title; ?> </a></h2>
</div>
<h2 class="h2title_thongbao">Thời gian: từ <?php echo date( 'j/m/Y',strtotime($arwrk[0]['C_BEGIN_DATE'])) ?> - <?php echo  date( 'j/m/Y',strtotime($arwrk[0]['C_END_DATE'])) ?>    <span class="pdatetime"> <?php echo $timenow; ?> <img src="../images/index/img_printer.jpg" /></span> </h2>
