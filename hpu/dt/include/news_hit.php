           ...
           <ul class="tabs tabs-3">
                <?php for($i=0;$i<$rowswrk;$i++) { 
                         ...
                         if ($arwrk[$i]['TYPE'] == 'Tin bài')
                                 {
                                 $url ="Doanthechitiettintuc-".$arwrk[$i]['PK_TINBAI_ID']."-".$obid."-".$subid."-".$fk_congty."-".changeTitle($arwrk[$i]['C_TITLE']).".html";
                                 }
                                 else 
                                 {
                                 $url ="Doanthechitietthongbao-".$arwrk[$i]['PK_TINBAI_ID']."-".$obid."-".$subid."-".$fk_congty."-".changeTitle($arwrk[$i]['C_TITLE']).".html";   
                                 }    
                         ?> 
		<li class="">
			<div class="tab-content" id="tab1">
                           ...
                           if($arwrk[$i]['C_FILEANH_MAIN'] == null) $file_anh = $arwrk[$i]['C_FILEANH_LEVEL']; else $file_anh = $arwrk[$i]['C_FILEANH_MAIN'];
                                    ...
                                    $img = $file_anh;
                           ?>   
                            <a href="<?php echo $url; ?>" title="<?php echo ew_HtmlEncode($arwrk[$i]['C_TITLE'])?>" > <img style="width:1000px;height:406px" src="<?php echo ew_HtmlEncode($img) ?>"  alt="<?php echo ew_HtmlEncode($arwrk[$i]['C_TITLE'])  ?>"/> </a>
